...
class Spider(Spider):
    ...
    # 候选域名：奇优影院频繁换域名，init 时自动探测第一个可用的
    # 已知死亡：qiyoudy5.com 及 qiyoudy1-4/6 系列（2026-09 已失效）
    # qivod.com 挂在 Cloudflare 后面，部分网络环境会触发人机验证
    HOST_CANDIDATES = [
        "https://www.qivod.com",
        "https://qiyoudy5.com",
        "http://qiyoudy5.com",
    ]
    # 命中这些标记说明不是真实站点（CF 人机验证 / 建站默认页）
    BAD_MARKS = ("Just a moment", "站点创建成功", "安全验证", "cf-browser-verification", "cf_challenge")

    def _probe_host(self):
        ua = "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36"
        for h in self.HOST_CANDIDATES:
            try:
                rsp = self.fetch(h + "/", headers={"User-Agent": ua, "Referer": h + "/"})
                html = rsp.text if hasattr(rsp, "text") else str(rsp)
                if html and ("stui-vodlist" in html or
                             ("<title>" in html and not any(b in html for b in self.BAD_MARKS))):
                    return h
            except:
                continue
        return self.HOST_CANDIDATES[0]

    def init(self, extend):
        try:
            self.HOST = self._probe_host()
        except:
    ...
    def categoryContent(self, tid, pg, filter, extend):
        ...
        try:
            ...
            url = f'{self.HOST}/list/{tid}_{pg}.html?order={order}'
            rsp = self.fetch(url)
            root = self.parse_html(rsp.content)
    ...
    def playerContent(self, flag, id, vipFlags):
        try:
            ...
            # 多种方式查找播放地址
            # 1. API链接 (注: api.yongfan99.com:81 已失效, 仅作兜底尝试)
            ...
            return {"parse": 1, "playUrl": "", "url": f"{self.HOST}{id}", "header": {
                "User-Agent": "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36",
                "Referer": self.HOST + "/",
            }}
