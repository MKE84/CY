...
# [数据根统一 v6+] 旧版把数据散落在「APP 私有缓存目录 / 脚本目录 / 本地包目录」三处：
# 清一次 APP 缓存主配置就没了，脚本目录还留下 cache、备份目录索引、默认本地包等残渣，
# 恢复逻辑不得不遍历一堆历史位置。现在全部收口到唯一的「配置备份目录」（下称数据根）：
#
#   <数据根>/
#     ├── persistent_config.json    主配置（唯一一份，含运行时状态）
#     ├── map/                      接口新代码（绝对路径化后的产物）
#     ├── cache/                    临时缓存（片段编辑等）
#     ├── backups/                  备份（一次备份 = 一个文件夹，含配置 + map）
#     └── .backup_dirs.json         历史用过的备份目录索引
#
# 清理数据 = 删除整个数据根；恢复默认 = 清数据根里除 backups/ 之外的东西。
# 旧版写在 APP 缓存目录 / 脚本目录下的数据一律废弃，不再读取、不再兼容。
DEFAULT_CONFIG_BACKUP_DIR = "/storage/emulated/0/本地包管理"

# 临时缓存的兜底位置（仅在数据根不可写时启用，优先级最低）
try:
    import tempfile as _tempfile
    CACHE_DIR = os.path.join(_tempfile.gettempdir(), 'zxb_cache')
except Exception:
    CACHE_DIR = os.getcwd()

BACKUP_DIR_INDEX_NAME = ".backup_dirs.json"


def _backup_dir_index_path():
    """备份目录索引的固定落点：<默认数据根>/.backup_dirs.json

    为什么不放在当前数据根里：用户如果把「配置备份目录」设到了 APP 私有目录
    （/storage/emulated/0/Android/data/<包名>/ 之类），清除 APP 数据或缓存时
    系统会把整个目录删掉——主配置没了，「配置备份目录」这个设置项本身也跟着没了，
    下次启动回到默认目录，既往成果就再也找不回来。
    索引固定在 SD 卡公共目录下，清 APP 缓存 / 清数据都删不到它，
    于是启动时仍知道"以前用过哪些备份目录"，能自动把配置捞回来。
    """
    try:
        d = os.path.abspath(DEFAULT_CONFIG_BACKUP_DIR)
    except Exception:
        return None
    try:
        ...
        pass
    return os.path.join(d, BACKUP_DIR_INDEX_NAME)
...
# [废弃] _get_app_cache_dir / _cache_root / PERSISTENT_CONFIG_PATH 已移除。
# 主配置不再落在 APP 私有缓存目录（清缓存即丢），统一改由
# Spider._data_root() + Spider._persistent_config_path() 动态给出。

def _decode_bytes(raw):
    """把字节解码成文本；非字节入参不再抛异常。

    [修复] 上游拿到 dict / list / int / None 时（例如接口返回了结构化数据，
    或某个字段被 JSON 解码过），原来的 raw[:3] 会直接抛 TypeError 穿透主流程。
    现在：已经是 str 就原样返回，其它类型统一走 str() 兜底。
    """
    if not raw:
        return ''
...
LOG_VIEW_MAX_CHARS = 15000
LOG_VIEW_TRIM_TO = 9000
...
def _as_url_str(v):
    ...
    None / dict / list / int 这些脏数据一律转空串，绝不让 urljoin 抛
    TypeError 穿透到主流程；bytes 按 utf-8 容错解码。
    """
    if v is None:
        return ""
    if isinstance(v, str):
...
def _collapse_path_slashes(path):
    """把路径里连续的 '/' 折叠成单个 '/'。"""
...
def _norm_url_slashes(url):
    """只归一化 path 段的重复斜杠。

    scheme 后面的 '//'（如 http://）不参与处理，因为它在 netloc 之前，
    不属于 path，天然安全。
    """
    try:
        ...
        return url
    if not p.path or "//" not in p.path:
        return url
    new_path = _collapse_path_slashes(p.path)
    if new_path == p.path:
        return url
    try:
...
def safe_urljoin(base, rel):
    """安全拼接 URL：脏数据不抛异常，结果不含三斜杠。

    [修复] 旧实现用一个占位串把 base 路径里原有的 "//" 保护起来，拼完再换回。
    它有两个毛病：
      1) 占位串若真实出现在 URL 里会被误替换；
      2) 保护之后 urljoin 把 "//" 当普通字符处理，base 含 "///" 时拼出
         http://a.com///x/y.json 这类三斜杠，部分站点直接 404；
         而 "http://a.com/x//" + "y.json" 又会丢掉一层目录。
    urljoin 本身对重复斜杠是安全的，真正要收拾的是**拼接结果**里多余的
    路径斜杠，因此改为：正常 join，再统一折叠 path 中的 "//"。
    """
    b = _as_url_str(base)
    r = _as_url_str(rel)
    if not r:
        return base if isinstance(base, str) else b
    if not b:
        return rel if isinstance(rel, str) else r
    low = r.strip().lower()
    if low.startswith(('http://', 'https://', 'file://', 'ftp://', 'data:')):
        return rel if isinstance(rel, str) else r
    try:
        joined = urllib.parse.urljoin(b, r)
    except Exception:
        return rel if isinstance(rel, str) else r
    return _norm_url_slashes(joined)
...
# [显示截断 v6+] UI 上"太长就省略号"的阈值。
# 原值经常把刚生成的数据（接口名、地址、状态说明）截得看不出是什么，
# 现在统一放大 1.5 倍（向上取整）；只影响"显示几多个字"，不影响内部逻辑与落盘内容。
UI_TRUNC_NAME = 36          # 生成的接口名 / 包名（原 24）
UI_TRUNC_SHORT = 45         # 设置项摘要、状态消息（原 30）
UI_TRUNC_URL = 60           # 地址类摘要（原 40）
UI_TRUNC_TAIL = 90          # 只保留尾部时的长度（原 60）
UI_TRUNC_TOAST = 300        # 应用内提示总长（原 200）
UI_TRUNC_TOAST_SHOWN = 180  # Toast 真正渲染的长度（原 120）
...
# [备份改为文件夹] 一次备份 = <数据根>/backups/down_config[<yymmddHHMMSS>]/，
# 里面放 persistent_config.json + map/，恢复时整体搬回，
# 才能把配置与接口新代码一起还原到备份前的状态（旧版单个 JSON 装不下 map）。
CONFIG_BACKUP_PREFIX = "down_config["
CONFIG_BACKUP_STAMP_FMT = "%y%m%d%H%M%S"
CONFIG_BACKUP_STAMP_RE = None

# 数据根下的固定子目录名
CONFIG_BACKUP_SUBDIR = "map"
DATA_CACHE_SUBDIR = "cache"
DATA_BACKUPS_SUBDIR = "backups"
...
SETTING_SPECS = [
    ...
    },
    {
        ...
        "icon": "✈️", "label": "User‑Agent", "fmt": "trunc_short",
    },
    {
        ...
        "icon": "✨️", "label": "GitHub 加速代理", "fmt": "trunc_short",
    },
    {
        "key": "proxy", "id": "proxy",
    ...
    },
    {
        ...
        "hint": "输入外部解密API URL",
        "icon": "✴️", "label": "备用解密接口", "fmt": "trunc_short",
    },
    {
        "key": "config_backup", "id": "config_backup",
        "title": "设置数据目录（配置备份目录）", "kind": "dir",
        "path": "config_backup_dir", "attr": "config_backup_dir",
        "normalize": "dir_slash", "makedirs": True,
        "hint": "主配置、接口新代码、临时缓存、备份全部放在这里；"
                "留空则用默认 %s" % DEFAULT_CONFIG_BACKUP_DIR,
        "icon": "💾", "label": "数据目录（备份根目录）", "fmt": "or_unset",
        "default": "",
    },
    {
    ...
    {
        "key": "local_url_format", "id": "local_url_format",
        "title": "本地路径格式", "kind": "choice",
        "path": "local_url_format", "src": "config", "default": "auto",
        "opts": [("auto", "自动（沿用现状，推荐）"),
                 ("file", "统一为 file:// 绝对路径"),
                 ("service", "统一为本机文件服务地址")],
        "icon": "🔗", "label": "本地路径格式", "fmt": "or_unset",
        "pre_hint": "只影响**新生成**的本地地址（接口 api、配置文件、复制出去的地址）；\n"
                    "读取时三种写法一律认得，历史数据不受影响。\n"
                    "「自动」= 各处沿用原有格式，与改动前完全一致；\n"
                    "「文件服务」= http://127.0.0.1:<端口>/file/<路径>，端口变了也能认出来。",
    },
]
...
class UIKit(object):
    ...
    def edit_scrollable(self, view):
        """让多行文本编辑框「能滚、能滑、内容不藏」，但绝不去碰滚动条绘制。

        ⚠️ Android 15（SDK 35）血泪教训：
        任何滚动条 API 都可能把系统内部 ScrollBarDrawable 打成 null，
        View.onDrawScrollBars 绘制时直接 NullPointerException 崩溃：
            × setScrollbarThumbDrawable(自定义滑块)   —— 实测必崩
            × setScrollBarSize / setScrollBarStyle / setScrollbarFadingEnabled
            × setVerticalScrollbarPosition / setVerticalScrollBarEnabled
            × setHorizontalScrollBarEnabled
        这些在别的 ROM 上没事，在 Android 15 上就是雷。滚动条长什么样、
        画不画，一律交给系统，一行都别改。

        这里只做三件纯"行为"的事（都不触碰绘制）：
          1) 去掉末尾省略号 —— 省略号会把内容截断，且让人误以为没有下文；
          2) 没有 MovementMethod 时补一个 —— 保证只读态也能滚；
          3) 触摸时让父容器别抢事件 —— 编辑框嵌在可滚动弹窗里照样能滑。
        全部包在 try 里：任一 ROM 不支持就跳过，不影响输入框本身。
        """
        try:
        ...
        try:
            SMM = self.j("android.text.method.ScrollingMovementMethod")
            if SMM is not None and view.getMovementMethod() is None:
                view.setMovementMethod(SMM.getInstance())
        except Exception:
            pass
        ...
        try:
            ...
            class _Touch(dynamic_proxy(ViewCls.OnTouchListener)):
            ...
            def _on_touch(v, ev):
                try:
                    ...
                    p = v.getParent()
                    if p is not None:
                        # 按下 / 滑动时别让外层 ScrollView 把事件抢走（0=DOWN，2=MOVE）
                        p.requestDisallowInterceptTouchEvent(a in (0, 2))
                except Exception:
                    pass
                # 返回 False：事件仍交回编辑框自己处理（滚动 / 选词 / 光标）
                return False
    ...
    def input(self, hint="", value="", multiline=False, mono=False,
        ...
        if multiline:
            ...
            try:
                ...
                pass
            # [修复] 多行编辑框不要末尾省略号：它只在末尾画个"…"，
            # 同时会把内容截断、滑动也滑不动。改成滚动显示，
            # 超出部分靠手指滑动查看（滚动条交给系统，不可自定义）。
            self.edit_scrollable(et)
        else:
            et.setSingleLine(True)
            # [修复] 单行输入框也要能左右滑：长路径 / 长地址只是被折叠在框里，
            # 看不到后半截又没法滑，等于把内容藏了一半。
            # 开启横向滚动后，光标移动会自动跟着滚动，末尾省略号只在"没到末尾"时显示。
            # 注意：只开滚动行为，不碰任何 ScrollBar API（Android 15 上会崩）。
            try:
...
class FileDownloader:
    ...
    def __init__(self, output_dir, config=None, log_callback=None, progress_callback=None, cancel_event=None):
        ...
        # [修复] 只有"远端返回 304 / 大小一致"这一条路径会往 unchanged 里塞，
        # 而"文件已存在直接跳过"这几条路径只写了 downloaded，没登记 unchanged，
        # 于是 fresh_count = len(downloaded) - len(unchanged) 把跳过也算成新下载，
        # 处理信息里的"下载文件数"虚高、"未变更数"几乎恒为 0。
        # 这里用一个去重集合统一登记，任何"本次没有真正产生新字节"的分支都调用它。
        self._unchanged_seen = set()

        # 文本下载缓存：{ (绝对地址, 是否强制解密): 文本 }，见 download_text
        self._text_cache = {}
    ...
    def _mark_unchanged(self, target_rel):
        """登记一个"本次没有产生新字节"的目标，保证统计不重复也不遗漏。

        同一个 target_rel 只登记一次（幂等），unchanged 与 downloaded 一一对应，
        这样 fresh_count 才是真正的"本次新下载数"。
        """
        try:
            key = str(target_rel)
        except Exception:
            return
        if not key or key in self._unchanged_seen:
            return
        self._unchanged_seen.add(key)
        try:
    ...
    def _own_hosts(self):
        """本机网卡 IP 集合（用于判断某个地址是不是本机架设的服务）。

        结果缓存，整个生命周期只解析一次；解析失败就是空集合，
        最坏情况只是"识别不出本机 IP"，不影响任何下载行为。
        """
        cached = getattr(self, "_own_hosts_cache", None)
        if cached is not None:
            return cached
        hosts = set()
        try:
            ...
            for _ip in socket.gethostbyname_ex(socket.gethostname())[2]:
                hosts.add(str(_ip).strip().lower())
        except Exception:
            pass
        self._own_hosts_cache = hosts
        return hosts

    def _is_own_service_url(self, url):
        """是否指向本机架设的服务（file:// 或 127.0.0.1 / localhost / 本机 IP）。

        这类地址本来就是本地文件服务，统计"回源引用"时要排除掉——
        否则把已经本地化好的地址也算成"需要联网"，数字会虚高。
        """
        try:
        ...
        if host in ('localhost', 'localhost.localdomain', '::1') or \
                host.startswith('127.'):
            return True
        return host in self._own_hosts()
    ...
    def resolve_url(self, rel_path, base_url, keep_own_proxy=False):
        # [修复] 脏数据（dict / list / int / None）不该在这里抛 AttributeError：
        # 配置里的地址字段偶尔会被上位机写成非字符串，原来会直接穿透主流程。
        ...
        with self._lock:
            if target_rel in self._processed:
                ...
                if os.path.exists(target_abs):
                    ...
                    self._mark_unchanged(target_rel)
                    return target_rel
    ...
    def _text_cache_key(self, url, base_url, force_decrypt):
        """文本下载缓存键：用**解析后的绝对地址**当键。

        这样 "a.json"、"../a.json"、"http://x.com/a.json" 只要指向同一份文件，
        就会命中同一份结果；force_decrypt 不同则分开存，避免拿错明文/密文。
        """
        try:
            full = self.resolve_url(self.split_url_and_suffix(url)[0], base_url)
        except Exception:
            full = None
        if not full:
            full = url
        return (str(full), bool(force_decrypt))

    def download_text(self, url, base_url, force_decrypt=None, _direct_only=False,
                      _no_more_alts=False):
        """带缓存的文本下载（对外入口）。

        [修复] 递归引用的 JSON（A 引 B、B 又引 A，或几十个接口引同一份公共配置）
        在同一次本地化里会被反复请求同一个地址——增量更新对它完全失效，
        每轮都要重新把这些文件拖一遍。
        这里按解析后的绝对地址缓存结果：
          · 命中就直接返回，不再发请求；
          · 备用地址重试（_direct_only / _no_more_alts）不查也不写缓存，
            否则重试会一直拿到同一份失败结果；
          · 失败返回 None 时不写缓存，下次照常重试。
        """
        if _direct_only or _no_more_alts:
            return self._download_text_uncached(
                url, base_url, force_decrypt, _direct_only, _no_more_alts)
        if not url or not isinstance(url, str):
            return None
        key = None
        try:
            key = self._text_cache_key(url, base_url, force_decrypt)
        except Exception:
            key = None
        if key is not None:
            try:
                ...
                hit = None
            if hit is not None:
                self._log(f"♻️ 复用已下载文本: {url}", level='debug')
                return hit
        text = self._download_text_uncached(
            url, base_url, force_decrypt, _direct_only, _no_more_alts)
    ...
    def _download_text_uncached(self, url, base_url, force_decrypt=None,
                                _direct_only=False, _no_more_alts=False):
        """真正发起请求的文本下载（原来的 download_text 实现）。"""
...
class Spider(BaseSpider):
    ...
    # 记录"上次启动时外部配置文件实际给出的键值"，用于判断用户是否手改过外部文件。
    # 不在 CONFIG_KEY_MAP 里，读写时键名原样保留。
    EXT_APPLIED_KEY = "_ext_applied"

    # 「管理入口」脚本路径的固化键：首次解析出可靠的回归地址后写进主配置，
    # 之后注入产物、切换接口一律沿用这个地址，不再随脚本运行位置或 config_file 变化。
    # 不在 CONFIG_KEY_MAP 里，读写时键名原样保留。
    MANAGER_SCRIPT_PATH_KEY = "manager_script_path"

    # 「注代」功能默认写入的代理地址（可在弹窗里修改，修改后记入配置）
    DEFAULT_FRAG_PROXY = "http://127.0.0.1:2080"
    ...
    frag_proxy_default = _cfg_prop('frag_proxy_default', '')
    ...
    def __init__(self):
        ...
        self.log_level = 'info'
        # [统一存储] 默认日志目录跟着数据根，不在脚本目录下建目录
        self.log_dir = os.path.join(DEFAULT_CONFIG_BACKUP_DIR, 'log')
    ...
    def _load_root_dirs(self):
        try:
            _cfg_path = self._persistent_config_path()
            if os.path.exists(_cfg_path):
                ...
                if not self.root_dirs:
                    default_dir = self.download_output_dir or os.path.join(self._data_root(), "本地包")
    ...
    def _strip_file_scheme(self, path):
        """把 file://<绝对路径> 还原成 os.path 认得的本地路径。

        [为什么需要] 对外（产物里的 api / ext.config_file）统一写的是
            file:///storage/emulated/0/123云盘/转本地包/down_config_v6.json
        这种**不转码**的明文地址。壳子会原样把它塞回 ext.config_file 传给我们，
        但读取链路只把 http/https/ftp 当远程，file:// 落进"本地路径"分支后：
            os.path.isabs("file:///storage/...")  → False
            os.path.exists("file:///storage/...") → False
        _resolve_file_path 于是拿这个带前缀的字符串去 base 目录拼，
        拼出来的全是无效路径，最终报「配置文件未能加载」。

        [做法] 在所有读取入口统一剥掉 file:// 前缀并解百分号编码，
        后面的逻辑才拿得到真实路径。非 file:// 地址原样返回。
        """
        try:
            s = str(path or "").strip()
        except Exception:
        ...
        try:
            ...
            pass
        # file:// 后没接根路径（如 file://storage/...）时补回斜杠；
        # Windows 盘符（file://C:/xxx）保持原样
    ...
    def _resolve_resource_path(self, source):
        if not source:
            return None, None
        source = self._strip_file_scheme(source.strip())
    ...
    def _log_dir(self):
        return (getattr(self, 'log_dir', None)
                or os.path.join(self.download_output_dir or self._data_root(), 'log'))
    ...
    def _fs_start_dir(self, typed, fallbacks=None, mode="dir"):
        """文件/文件夹弹窗的起始目录：优先落在输入框里的有效路径上。

        [要求] 避免每次都从根目录一层层点进去：
          · 输入框是已存在的目录 → 直接用它；
          · 是还没建的目录 → 用它最近的存在父目录（方便就地新建）；
          · 选文件模式下填的是文件路径 → 用该文件所在目录；
          · 都没有 → 依次退回 fallbacks（下载目录 / 已添加的目录…）。
        """
        v = str(typed or "").strip()
        if v:
            try:
                p = v if os.path.isabs(v) else os.path.abspath(v)
            except Exception:
                p = v
            try:
    ...
    def _ui_site_card(self, kit, name, subtitle, checked, on_toggle, actions, dim=False,
                      per_row=None, show_switch=True, refs=None):

        # [修复] 卡片上的开关 / 标题原先只挂在 Java View 的属性上
        # （card._switch / card._title_view），部分机型 / 代理对象上赋值会静默失败，
        # 导致"全选 / 反选 / 清空"后拿不到开关、形态不同步。
        # 这里额外用一个**纯 Python 字典**把引用交回调用方，100% 可靠。
        if refs is None:
            refs = {}
        G = kit.gravity()
    ...
    def _load_default_config(self):
        # [统一存储] 默认输出目录跟着数据根走，不在脚本目录下建目录
        default_output = os.path.join(self._data_root(), "本地包")
        return {
            "sources": [],
    ...
    def _apply_config(self, config):
        ...
        if not self.download_output_dir:
            self.download_output_dir = os.path.join(self._data_root(), "本地包")
    ...
    LOCAL_URL_AUTO = "auto"
    LOCAL_URL_FILE = "file"
    LOCAL_URL_SERVICE = "service"

    LOCAL_URL_FORMATS = (LOCAL_URL_AUTO, LOCAL_URL_FILE, LOCAL_URL_SERVICE)

    def _local_url_format(self):
        """当前选择的本地路径格式（auto / file / service）。

        auto（默认）= 各调用点沿用自己原来的格式，行为与改动前完全一致，
        只有用户明确选了 file / service 才会统一改写。
        """
        v = str(getattr(self, 'local_url_format', '') or '').strip().lower()
        if v in self.LOCAL_URL_FORMATS:
            return v
        try:
            v2 = str((self.config or {}).get('local_url_format') or '').strip().lower()
        except Exception:
            v2 = ''
        return v2 if v2 in self.LOCAL_URL_FORMATS else self.LOCAL_URL_AUTO

    def _local_url(self, abs_path, default_scheme="file"):
        """把本地绝对路径转成对外的地址，按设置统一格式。

        auto    —— 用调用点原本的格式（default_scheme），与旧行为一致；
        file    —— 统一成 file://<绝对路径>；
        service —— 统一成本机文件服务 http://127.0.0.1:<端口>/file/<绝对路径>，
                   探测不到端口时退回 file://，保证地址一定可用。
        """
        _raw = str(abs_path or "").strip()
        # 远程地址（如 http://pg.top/py/xxx.py）不能进 os.path.abspath，
        # 否则会被拼上当前工作目录变成 /sdcard/http:/pg.top/... 这种废地址。
        if _raw.lower().startswith(("http://", "https://")):
            return _raw
        # 已经带 file:// 前缀的（拼接结果 / 壳子回传值）先剥掉再走 abspath，
        # 否则 "file:///storage/..." 会被当成相对路径拼上工作目录，变成
        # /data/workspace/file:/storage/... 这种废地址。剥掉后本方法可重复调用。
        if _raw.lower().startswith("file://"):
            try:
                _raw = urllib.parse.unquote(_raw[7:])
            except Exception:
                _raw = _raw[7:]
        try:
            p = os.path.abspath(_raw)
        except Exception:
            p = _raw
        if not p:
            return ""
        fmt = self._local_url_format()
        if fmt == self.LOCAL_URL_FILE:
            return "file://" + p
        if fmt == self.LOCAL_URL_SERVICE:
            try:
                base = str(self._effective_file_service_base() or "").rstrip('/')
                if base.startswith(('http://', 'https://')):
                    # p 本身以 / 开头，base 以 /file 结尾，直接拼即可（别再补斜杠）
                    return base + urllib.parse.quote(p, safe='/')
            except Exception:
                pass
            return "file://" + p
        return ("file://" if default_scheme == "file" else default_scheme) + p

    def _local_url_to_path(self, url):
        """把本地地址还原成绝对路径：file:// 与 http://127.0.0.1:<任意端口>/file/
        两种写法一律认得（端口换了也能认出来）。

        不是本地地址就返回空串，调用方按"远程地址"处理即可。
        """
        try:
            s = str(url or "").strip()
        except Exception:
        ...
        try:
            ...
            return ""
        if not (host in ('localhost', '::1') or host.startswith('127.')):
            return ""
        # /file/<绝对路径> —— 端口忽略，重启后端口变了照样认得出
        seg = p.path or ""
        marker = '/file'
        i = seg.lower().find(marker)
        if i < 0:
            return ""
        # 保留路径自带的前导 "/"，还原出来才是 /sd/a.json 而不是 sd/a.json
        rest = seg[i + len(marker):]
        if not rest.startswith('/'):
            return ""
        return urllib.parse.unquote(rest)

    def _service_url_to_path(self, url):
        """把本机文件服务地址还原成真实绝对路径（含服务根前缀推断）。

        服务的根在不同 ROM 上不一样：多数是 /storage/emulated/0，有的就是 /，
        所以按候选前缀逐个拼，取**文件确实存在**的那一个；
        一个都不存在就返回空串，交给调用方兜底。
        """
        rel = self._local_url_to_path(url)
        if not rel:
            return ""
        rel = rel.lstrip('/')
        try:
            cands = list(self.FILE_SERVICE_PREFIX_CANDIDATES)
        except Exception:
            cands = ['/storage/emulated/0', '/sdcard', '']
        for prefix in cands:
            try:
                p = os.path.abspath(os.path.join(prefix, rel) if prefix
                                    else ('/' + rel))
            except Exception:
                continue
            try:
    ...
    def _to_ext_config_url(self, path):
        """对外暴露的配置文件地址：统一成**不转码**的本地绝对路径。

        [路径格式统一] 框架注入的 _config_file_path 通常是
            http://127.0.0.1:<端口>/file/123%E4%BA%91%E7%9B%98/.../down_config_v6.json
        而产物里其它本地地址是
            file:///storage/emulated/0/本地包管理/本地包/12/js/douban.js
        两种写法混在一起：端口一变前一种就失效，编码过的中文也看不出是哪个目录。
        所以这里先把本机文件服务地址 / 已编码的 file:// 还原成真实路径，
        再按统一格式（默认 file:// + 原始路径，不做 percent 编码）输出。
        """
        v = str(path or "").strip()
        if not v:
            return v
        low = v.lower()
        if low.startswith(("http://", "https://")):
            if not self._local_url_to_path(v):
                return v          # 真·远程地址，原样保留
            # 本机文件服务地址：还原成真实路径；还原不出来就退回我们自己的
            # 主配置（一定是本地真实路径），保证产物里始终是统一、不转码的地址
            real = self._service_url_to_path(v) or self._persistent_config_path()
            return self._local_url(real)
        if low.startswith("file://"):
            try:
                return self._local_url(urllib.parse.unquote(v[7:]))
            except Exception:
                return v
        ...
        if not os.path.isabs(v):
            try:
                ...
                v = os.path.abspath(v)
        return self._local_url(v)

    @staticmethod
    def _strip_rel_suffix(abs_path, rel):
        """[路径关联] 若 abs_path 的末尾若干段与相对写法 rel 完全一致，返回剥离后的基目录。

        用途：已知 config_file 的真实绝对路径、以及它在 box.json 里的相对写法，
        就能反推出 box 根目录，进而算出与它成对的脚本路径：
            abs = /storage/emulated/0/box/json/down_config_v6.json
            rel = json/down_config_v6.json
            →   /storage/emulated/0/box/
        rel 是绝对路径、或末尾对不上时返回空串，由调用方自行兜底。
        """
        try:
            a = str(abs_path or "").replace("\\", "/")
            r = str(rel or "").replace("\\", "/").strip()
        except Exception:
            return ""
        if not a or not r:
            return ""
        # 只有相对写法才能据此反推根目录；绝对路径（含盘符 / scheme）无从下手
        if r.startswith("/") or "://" in r or (len(r) > 1 and r[1] == ":"):
            return ""
        a_segs = [x for x in a.split("/") if x]
        r_segs = [x for x in r.split("/") if x and x != "."]
        if not r_segs or len(a_segs) <= len(r_segs):
            return ""
        if a_segs[-len(r_segs):] != r_segs:
            return ""
        return "/" + "/".join(a_segs[:-len(r_segs)])

    def _manager_script_rel(self, raw_sites, cf_abs):
        """在 box 原始数据里挑出「指向本脚本」的那条 site，返回 (相对 api, 相对 config_file)。

        定位依据优先取 ext.config_file —— 脚本与配置在 box 里成对出现，两者的相对写法
        互为参照（可能同目录 ./a.py + ./b.json，也可能分层 ./py/a.py + ./json/b.json）；
        其次才退而按「api 文件名与当前脚本同名」匹配。找不到返回 ("", "")。
        """
        cf_name = os.path.basename(str(cf_abs or ""))
        self_name = os.path.basename(SELF_PATH) if SELF_PATH else ""
        fallback = ("", "")
        try:
            sites = list(raw_sites or [])
        except Exception:
            return fallback
        for s in sites:
            ...
            try:
                ...
                if isinstance(ext, dict):
                    raw_cf = str(ext.get("config_file") or "").strip()
                elif isinstance(ext, str) and ext.strip().startswith("{"):
                    raw_cf = str((json.loads(ext) or {}).get("config_file") or "").strip()
            except Exception:
                raw_cf = ""
            if cf_name and raw_cf and os.path.basename(raw_cf) == cf_name:
                return (api, raw_cf)
            if self_name and os.path.basename(api) == self_name and not fallback[0]:
                fallback = (api, raw_cf)
        return fallback

    def _manager_self_path(self):
        """已固化的管理入口脚本路径（首次运行时记下的「回归地址」）。

        没固化过返回空串，由调用方按原有逻辑现算。
        """
        try:
            return str(self.config.get(self.MANAGER_SCRIPT_PATH_KEY) or "").strip()
        except Exception:
            return ""

    def _freeze_manager_script(self, path):
        """记下管理入口的脚本路径——只记第一次，之后一律返回已固化的那个。

        [为什么固化] 脚本被复制 / 移动到别的目录运行后，config_file 与脚本自身路径
        都会变，注入到产物里的 api 跟着变，用户点「本地包管理」就可能指向另一份脚本、
        甚至一个根本不存在的路径。首次运行解析出的那份地址才是真正的「回归入口」，
        存下来之后：注入时用它、切换到其它接口后也用它，始终不变。

        [什么不固化] 框架实际执行的是 /data/user/0/<包名>/cache/py/<hash>.py 这类
        APP 私有缓存副本（清一次缓存就没了，文件名还是 hash），绝不能当入口；
        解析出的路径若文件不存在也不可信。这两类一律返回空串，交回调用方兜底，
        等下次拿到可靠地址再记。
        """
        _raw = str(path or "").strip()
        # 远程地址不能进 os.path.abspath（会被拼上工作目录），原样保留
        if _raw.lower().startswith(("http://", "https://")):
            p = _raw
        elif _raw.lower().startswith("file://"):
            # 明文 file:// 地址先剥前缀再判存在性，否则永远 isfile 失败、无法固化
            try:
                p = urllib.parse.unquote(_raw[7:])
            except Exception:
                p = _raw[7:]
        else:
            try:
        ...
        if not p:
            return ""
        low = p.replace("\\", "/").lower()
        # APP 私有目录 / 缓存副本：不能作为长期入口
        if low.startswith("/data/") or "/cache/py/" in low or "/app_py/" in low:
            return ""
        # 远程地址无法校验存在性（可能未联网），直接采信；本地地址必须真实存在
        if not low.startswith(("http://", "https://")):
            try:
                ...
                return ""
        saved = self._manager_self_path()
        if saved and (saved.lower().startswith(("http://", "https://"))
                      or os.path.isfile(saved)):
            return saved          # 已固化且仍然有效，永远返回它
        # 固化值指向的文件没了（脚本被移动 / 删除）→ 允许重新解析一次
        try:
            self.config[self.MANAGER_SCRIPT_PATH_KEY] = p
            self._save_persistent_config()
            self._log("[入口固化] 管理入口脚本地址 = %s" % p)
        except Exception:
            pass
        return p

    # 判断候选 .py 是否就是"本脚本"用的内容特征串（脚本里的管理入口 key）
    _SELF_SIGNATURE = "local_package_manager"

    def _file_md5(self, path, limit=None):
        try:
            h = hashlib.md5()
    ...
    def _find_self_script_in(self, cf_dir, max_up=2, max_depth=2):
        """在 config_file 所在目录找回本脚本的真实落点。

        [为什么需要] 框架执行的是 /data/user/0/<包名>/cache/py/<名>.py 这份缓存副本，
        直接把它写进产物，等于把入口指向 APP 私有缓存——清一次缓存接口就废了。
        config_file 是脚本自己写进产物的真实地址，它附近通常就放着脚本本体，
        只是名字未必相同（可能是下载时生成的 hash 名 cbf0e229....py）。

        [支持分层] api 与 config_file 常常不在同一层：
            ./py/在线转本地@v6[内测].py  +  ./json/down_config_v6.json
        脚本在 json/ 的兄弟目录 py/ 下，只盯 config_file 同目录必然扑空。
        所以从 config_file 目录逐级向上回溯，每层再向下做有限深度搜索。

        [定位顺序] 按可靠度从高到低，命中即止：
          1. 内容 MD5 与当前运行副本一致 —— 改名也认得，最准；
          2. 文件名与运行副本同名；
          3. 内容含本脚本特征串（改过内容 / 框架改写副本都认得）；
          4. 全部候选中只有一个 .py。
        都不中返回空串，交给后续逻辑兜底。
        """
        try:
            start = os.path.abspath(str(cf_dir or ""))
        except Exception:
            return ""
        if not start or not os.path.isdir(start):
            return ""

        # ---- 收集回溯起点：config_file 目录本身 + 往上若干层 ----
        roots = []
        d = start
        for _ in range(max_up + 1):
            if not d or d in roots:
                break
            low = d.replace("\\", "/").lower()
            # 缓存目录 / APP 私有目录不可能是脚本的真实落点
            if low.startswith("/data/") or "/cache/py/" in low:
                break
            roots.append(d)
        ...
        # ---- BFS 收集候选（深度受限，跳过无关目录） ----
        skip_names = {"map", "localized", self.COLLECTION_DIR_NAME,
                      "backups", "cache", "log"}
        cands = []
        seen = set()
        queue = [(r, 0) for r in roots]
        while queue and len(cands) < 200:
            ...
            seen.add(cur_abs)
            try:
                names = sorted(os.listdir(cur_abs))
            except Exception:
                names = []
            for n in names:
                p = os.path.join(cur_abs, n)
                if n.lower().endswith('.py'):
                    if os.path.isfile(p):
                        cands.append(p)
                elif os.path.isdir(p) and depth < max_depth:
                    if n.startswith('.') or n.lower() in skip_names:
                        continue
        ...
        try:
            if SELF_PATH and os.path.isfile(SELF_PATH):
                ...
                self_md5 = self._file_md5(SELF_PATH)
        except Exception:
            pass

        # 1) MD5 一致（先用体积过滤，省掉绝大多数无谓的哈希计算）
        if self_md5:
            for p in cands:
                try:
                    if self_size >= 0 and os.path.getsize(p) == self_size:
                        if self._file_md5(p) == self_md5:
                            return p
                except Exception:
                    continue
        # 2) 同名（优先挑离 config_file 最近的那一级目录）
        if self_name:
            for p in cands:
        ...
        for p in cands:
            try:
                with open(p, 'r', encoding='utf-8', errors='ignore') as fp:
                    head = fp.read(524288)
                if self._SELF_SIGNATURE in head:
                    return p
            except Exception:
                continue
        # 4) 只有一个候选
    ...
    def _find_self_site_from_box(self, box_url):
        """从当前生效的 box 里，读出「指向本脚本」那条 site 的 api / config_file 原值。

        [为什么需要] api 与 config_file 可能既不同目录、也不同域名：
            api         = http://pg.top/py/在线转本地@v6[内测].py
            config_file = http://hfr1107.top/json/down_config_v6.json
        这种跨站组合没法从 config_file 反推，但 box 里本来就把两者成对写着，
        直接读出来沿用即可。

        [匹配方式] 优先按 ext.config_file 的文件名与当前配置文件的文件名相同来定位
        （这是最稳的配对依据，脚本与配置在 box 里成对出现）；退而求其次按 api
        的文件名与当前脚本同名来匹配。

        [相对地址] box 里的 api 若写成 ./py/xxx.py 这类相对路径，按 box 所在目录
        拼成绝对地址；本来就是绝对地址（http/file）的原样返回。
        """
        box_url = str(box_url or "").strip()
        if not box_url:
            return ""
        try:
            data = self._load_json_resource(box_url, allow_decrypt=False)
        except Exception as e:
            self._log("[入口固化] 读取 box 失败 %s: %s" % (box_url, e),
                      level='debug')
            return ""
        ...
        cf_name = ""
        try:
            cf_name = os.path.basename(
                str(self._config_file_path or self._manager_config_file() or "")
                .replace("\\", "/").rstrip("/"))
        except Exception:
            cf_name = ""
        self_name = os.path.basename(SELF_PATH) if SELF_PATH else ""
        ...
        for s in sites:
            ...
            try:
                ...
                if isinstance(ext, dict):
                    ...
                    raw_cf = str((json.loads(ext) or {}).get("config_file")
                                 or "").strip()
            except Exception:
                raw_cf = ""
            if cf_name and raw_cf and os.path.basename(
                    raw_cf.replace("\\", "/").rstrip("/")) == cf_name:
                hit = api
                cf_hit = raw_cf
                break
            if not fallback and self_name and os.path.basename(
                    api.replace("\\", "/").rstrip("/")) == self_name:
                fallback = api
                cf_fallback = raw_cf
        return (hit or fallback, cf_hit or cf_fallback)

    def _resolve_ref_from_box(self, raw, box_url):
        """把 box 内的一条地址写法按「box 基准」解析成对外地址（api / config_file 通用）。

        [绝对写法原样] 写成 http://pg.top/py/xxx.py 的就原样返回 —— api 与
        config_file 完全可以在不同域名，各写各的，不做任何拼凑。
        本地绝对路径统一成明文 file://（不 percent 编码）。

        [相对写法按 box 解析]
            box 本地 file:///a/b/box.json  + ./py/xxx.py  → file:///a/b/py/xxx.py
            box 远程 http://x.top/box.json + ./py/xxx.py  → http://x.top/py/xxx.py
        即基准取 box 所在目录（去掉 box 文件名那段），与本地情形保持一致。
        中文一律保持明文，不做 percent 编码，便于一眼看出落在哪个目录。
        """
        raw = str(raw or "").strip().replace("\\", "/")
        if not raw:
            return ""
        low = raw.lower()
        # 绝对地址：原样（本地绝对路径统一成明文 file://）
        if low.startswith(("http://", "https://", "assets://", "proxy://")):
            return raw
        if low.startswith("file://"):
            return self._local_url(raw)
        if raw.startswith("/") or (len(raw) > 1 and raw[1] == ":"):
            return self._local_url(raw)

        # 相对写法：按 box 基准解析
        box = str(box_url or "").strip().replace("\\", "/")
        if not box:
            return ""
        blow = box.lower()
        if blow.startswith(("http://", "https://")):
            # 按 URL 路径段规范化，而不是简单字符串拼接：
            # 原来用 raw.lstrip("./")，会把 "../" 的前导点一起吃掉
            # （../py/x.py 被当成 py/x.py，平白多插一级目录）。
            try:
                m = re.match(r'^(https?://[^/]+)(/.*)?$', box)
                if not m:
                    return ""
                origin = m.group(1)
                segs = [s for s in (m.group(2) or "/").split("/") if s]
                if segs:
                    segs.pop()          # 去掉 box 文件名那一段
                for part in raw.split("/"):
                    ...
                    if part == "..":
                        ...
                        continue
                    segs.append(part)
                return origin + "/" + "/".join(segs)
            except Exception:
                head = box.rstrip("/")
                i = head.rfind("/")
                return (head[:i] if i > 0 else head) + "/" + raw.lstrip("./")
        if blow.startswith("file://"):
            box_path = box[7:]
        else:
            box_path = box
        try:
            base = os.path.dirname(box_path)
            joined = os.path.normpath(os.path.join(base, raw))
        except Exception:
            return ""
        return self._local_url(joined)

    def _resolve_self_api_from_box(self, box_url):
        """从 box 里读出「指向本脚本」那条 site 的 api，按 box 基准解析后返回。"""
        api, _cf = self._find_self_site_from_box(box_url)
        return self._resolve_ref_from_box(api, box_url)

    def _resolve_self_cf_from_box(self, box_url):
        """同一条 site 的 ext.config_file，按同样的规则解析后返回。

        与 api 成对出现、同样可能跨目录跨域名，所以共用一套解析：
        绝对写法原样（http://hfr1107.top/json/xxx.json），相对写法按 box 目录拼。
        """
        _api, cf = self._find_self_site_from_box(box_url)
        return self._resolve_ref_from_box(cf, box_url)

    def _manager_config_file_url(self):
        """注入到产物 ext.config_file 的地址：优先沿用 box 里的写法。"""
        try:
            _box = str(getattr(self, "_original_oktv_url", "") or "").strip()
            if _box:
                _cf = self._resolve_self_cf_from_box(_box)
                if _cf:
                    return _cf
        except Exception:
            pass
        return self._to_ext_config_url(self._manager_config_file())

    def _manager_script_url(self, config_file_url=None, raw_sites=None):
        """管理入口的 api 地址：由 ext.config_file 的真实路径反推，不用脚本自身路径。

        [路径统一 · 不转码] 旧写法直接沿用 config_file 的 URL 目录部分，产物里就出现
            http://127.0.0.1:9978/file/123%E4%BA%91%E7%9B%98/%E8%BD%AC%E6%9C%AC%E5%8C%B0%E5%8C%85/xxx.py
        这类百分号编码地址：端口一变就失效，中文也看不出是哪个目录，和同一份产物里的
        其它本地地址（file:///storage/emulated/0/本地包管理/本地包/12/js/douban.js）
        完全是两种形态。

        [路径关联] 更不能拿脚本自身的运行路径（SELF_PATH）硬拼 —— 框架实际跑的是
        /data/user/0/<包名>/cache/py/<hash>.py 这份副本，把它写进产物，等于把入口指向
        APP 私有缓存，清一次缓存接口就废了。
        正确来源是 config_file：注入的脚本与 config_file 在 box 里成对出现，
        用「config_file 的真实绝对路径」+「两者在 box 里的相对写法」即可反推出脚本位置，
        同目录（./a.py + ./b.json）与分层（./py/a.py + ./json/b.json）都对得上。
        """
        if not SELF_PATH:
            return ""
        cf = config_file_url
        ...
        # [首选] box 里怎么写的就怎么注入：绝对写法原样（可跨目录 / 跨域名），
        # 相对写法按 box 所在目录拼。这是唯一"用户显式表达过意图"的来源，
        # 优先于任何基于磁盘的猜测 —— 改了 box 里的地址，注入结果必须跟着变。
        try:
            _box = str(getattr(self, "_original_oktv_url", "") or "").strip()
            if _box:
                _api = self._resolve_self_api_from_box(_box)
                if _api:
                    # 只在还没固化过时记一笔（离线 / 无回归地址时兜底用）；
                    # 返回值一律以 box 写法为准 —— 不能让旧固化值盖掉用户新改的地址。
                    self._freeze_manager_script(_api)
                    return self._local_url(_api)
        except Exception:
            pass

        # [次选] 已固化的回归地址：box 读不到（离线 / 匹配不到 / 没设回归地址）时
        # 用它兜底，保证注入的仍是那份真实脚本，而不是 APP 私有缓存副本。
        _fixed = self._manager_self_path()
        if _fixed and (_fixed.lower().startswith(("http://", "https://"))
                       or os.path.isfile(_fixed)):
            return self._local_url(_fixed)

        # [再次] 在 config_file 的真实目录附近找回脚本本体（MD5 / 同名 / 特征串）。
        try:
            cf_abs0 = self._local_url_to_path(cf) or ""
            if not cf_abs0 and os.path.isabs(cf):
                cf_abs0 = cf
            if cf_abs0:
                _hit = self._find_self_script_in(os.path.dirname(cf_abs0))
                if _hit:
                    return self._local_url(
                        self._freeze_manager_script(_hit) or _hit)
        except Exception:
            pass

        # 次选：由 config_file 的真实路径 + 相对写法反推脚本位置（并对存在性做校验）
        try:
            cf_abs = self._local_url_to_path(cf) or ""
            if cf_abs and os.path.isabs(cf_abs):
                rel_api, rel_cf = self._manager_script_rel(raw_sites, cf_abs)
                if rel_api:
                    base = (self._strip_rel_suffix(cf_abs, rel_cf)
                            or os.path.dirname(cf_abs))
                    _cands = [
                        os.path.abspath(os.path.join(base, rel_api)),
                        os.path.join(os.path.dirname(cf_abs),
                                     os.path.basename(rel_api)),
                    ]
                    for _c in _cands:
                        try:
                            if os.path.isfile(_c):
                                return self._local_url(
                                    self._freeze_manager_script(_c) or _c)
                        except Exception:
                            continue
        except Exception:
            pass
        ...
        if low.startswith(("http://", "https://")):
            try:
                # 先按"文件确实存在"还原真实路径（服务根在不同 ROM 上不同，需逐个前缀探测）
                real = (self._service_url_to_path(cf) or "").strip()
                if real:
                    real_dir = os.path.dirname(real)
                    name = os.path.basename(SELF_PATH)
                    target = os.path.join(real_dir, name) if real_dir else name
                    return self._local_url(
                        self._freeze_manager_script(target) or target)
            except Exception:
                pass
            try:
                head, _sep, tail = cf.rpartition("/")
                if head:
                    # 兜底：还原不出真实路径时才沿用原目录拼接，与改动前行为一致
                    return head + "/" + os.path.basename(SELF_PATH)
            except Exception:
                pass
        _fallback = self._freeze_manager_script(SELF_PATH) or SELF_PATH
        if _fallback.replace("\\", "/").lower().startswith("/data/"):
            # 只剩缓存副本可选：入口是临时的，清一次缓存就失效，明确记一条日志便于排查
            self._log("[入口固化] 未能定位到脚本真实落点，暂用缓存副本: %s"
                      % _fallback, level='warn')
        return self._local_url(_fallback)

    def _config_fail_reason(self, path):
        """配置文件加载失败时，给出确切原因（而不是一句笼统的"常见原因"）。"""
        try:
            p = self._strip_file_scheme(str(path or "").strip())
        except Exception:
            return "路径无法解析。"
        if not p:
            return "未指定配置文件路径。"
        if self._is_remote_path(p):
            return ("远程地址拉取失败：\n"
                    "网络不通、链接失效，或返回内容不是合法 JSON。")
        try:
            exists = os.path.exists(p)
        except Exception:
            return "路径无法访问（无读取权限或路径非法）。"
        if not exists:
            return ("文件不存在。\n"
                    "多半是文件被移动/改名/删除，或它所在的云盘同步目录\n"
                    "还没把文件下载到本地。\n"
                    "可到「设置」把配置文件重新指向当前实际位置，\n"
                    "下次生成产物时就会写入新地址。")
        try:
            txt = _read_text_file(p)
        except Exception as e:
            return "文件存在但读取失败：%s" % e
        try:
            json.loads(txt)
        except Exception as e:
            return ("文件存在且能读，但 JSON 解析失败：\n%s\n"
                    "注释、尾随逗号、BOM、被截断的内容都可能造成，\n"
                    "日志里有具体行号。" % e)
        return ("文件存在、JSON 也合法，但解析结果为空或不被识别为配置。\n"
                "请确认文件内容是本程序的配置（而非接口 box 配置）。")

    def _manager_config_file(self):
        """对外暴露的配置文件路径（写进产物 ext.config_file 的那个）。

        [失效纠正] _config_file_path 记的是"上次运行时"的地址，一旦写进主配置就
        被无限沿用：文件搬过家 / 原目录被删 / 云盘同步目录里文件还没落本地之后，
        注入的仍是一条死链 —— 壳子照着去读，读不到就弹「配置文件未能加载」，
        而用户看弹窗里的路径往往又是"当初对、现在错"的那个。
        所以这里先校验存在性，失效就丢掉记录、改用当前数据根下的主配置。
        """
        real = (getattr(self, "_config_file_path", None) or "").strip()
        if real:
            try:
                p = self._strip_file_scheme(real)
                if self._is_remote_path(p) or os.path.isfile(p):
                    return real
            except Exception:
                return real
            try:
                self._log("[配置纠正] 记录的配置文件已失效，改用数据根主配置: %s"
                          % real, level='warn')
            except Exception:
                pass
            try:
                ...
                pass
        cands = []
        # [统一存储] 配置文件就是数据根下的 persistent_config.json
        try:
            for d in self._config_backup_dirs():
                cands.append(os.path.join(d, CONFIG_BACKUP_NAME))
        except Exception:
            pass
        for c in cands:
            try:
                ...
                continue
        return cands[0] if cands else self._persistent_config_path()
    ...
    def _snapshot_config(self, include_runtime=False):
        ...
        config = {
            ...
            # 管理入口脚本的固化地址（首次运行记下，之后注入 / 切接口一律沿用）
            self.MANAGER_SCRIPT_PATH_KEY: self._manager_self_path(),

            # 外部配置文件快照（判断用户是否手改过外部文件用）
            self.EXT_APPLIED_KEY: self.config.get(self.EXT_APPLIED_KEY),

            # 「注代」默认代理地址（接口改装中心写入片段用）
            "frag_proxy_default": getattr(self, "frag_proxy_default", "") or "",
        }
        if include_runtime:
            config.update({
    ...
    def _save_config_to_file(self, path=None, mirror=True):
        ...
        [统一存储] 旧版默认会额外写一份「APP 缓存目录/config.json」中文键快照，
        内容与主配置重复、且从不被读取，现已去掉 —— 只保留数据根下这一份主配置。
        显式传入 path 时按调用方要求写到指定位置（导出等场景），并顺带持久化一次。
        """
        if path is None:
            # 唯一一份主配置：直接走持久化流程（含运行时状态）
    ...
    def _save_persistent_config(self, mirror=True):
        with self._config_io_lock:
            try:
                ...
                path = self._persistent_config_path()
                self._write_json_atomic(path, snapshot)
                self._log(f"配置已持久化: {path}", level='debug')
            except Exception as e:
                self._log(f"持久化配置失败: {e}", level='error')
                snapshot = None

        if snapshot is not None:
            # [清缓存继承] 主配置写成功了，就把当前数据根记到缓存之外的索引里。
            # 记在这里而不是镜像之后：镜像循环会跳过"与主配置同路径"的那一份，
            # 数据根只有一个时 wrote 恒为 0，索引就永远落不下来。
            self._remember_backup_dirs()
    ...
    def _data_root(self):
        ...
        [统一存储] 主配置 / map / 临时缓存 / 备份全部放在它下面：
            <数据根>/persistent_config.json
            <数据根>/map/
            <数据根>/cache/
            <数据根>/backups/
        清理数据 = 删掉这个目录；恢复默认 = 清掉它里面除 backups/ 之外的东西。
        未显式设置 config_backup_dir 时用 DEFAULT_CONFIG_BACKUP_DIR。
        """
        d = str(getattr(self, 'config_backup_dir', '') or '').strip()
        if not d:
            d = DEFAULT_CONFIG_BACKUP_DIR
        try:
            d = os.path.abspath(d)
        except Exception:
            d = DEFAULT_CONFIG_BACKUP_DIR
    ...
    def _persistent_config_path(self):
        """主配置（唯一一份，含运行时状态）。"""
        return os.path.join(self._data_root(), CONFIG_BACKUP_NAME)

    def _data_cache_dir(self):
        """临时缓存目录（片段编辑等），落在数据根下；数据根不可写时退回系统临时目录。"""
        try:
            p = os.path.join(self._data_root(), DATA_CACHE_SUBDIR)
    ...
    def _backups_root(self):
        """备份存放根目录（一次备份 = 下面的一个文件夹）。"""
        return os.path.join(self._data_root(), DATA_BACKUPS_SUBDIR)

    def _config_backup_dirs(self):
        """保留原签名的兼容方法：现在数据根只有一个，返回单元素列表。"""
        try:
    ...
    def _remember_backup_dirs(self):
        """把当前正在用的备份目录记到缓存之外的索引里。

        只在真的写成功之后调用；记下来了，即使以后 APP 数据被清掉、
        「配置备份目录」这个设置项丢失，也还能顺着索引找到历史目录。
        """
        path = _backup_dir_index_path()
        if not path:
            return False
        try:
            dirs = []
            for d in self._config_backup_dirs():
                try:
                    dirs.append(os.path.abspath(str(d)))
                except Exception:
                    continue
            if not dirs:
                return False
            ...
            if os.path.isfile(path):
                try:
                    old = json.loads(open(path, 'r', encoding='utf-8').read())
                    if isinstance(old, dict):
                        idx = {str(k): v for k, v in old.items()
                               if isinstance(k, str)}
                except Exception:
                    idx = {}
            now = int(time.time())
            # 节流：10 分钟内已经记过、且目录没变就不用重写，省掉大量无谓 IO
            already = True
            for d in dirs:
                try:
                    if (now - int(idx.get(d) or 0)) >= 600:
                        already = False
                except Exception:
                    already = False
            if already and set(dirs) <= set(idx.keys()):
                return True
            for d in dirs:
                idx[d] = now
            # 最多留 20 条，避免索引无限膨胀
            if len(idx) > 20:
                idx = dict(sorted(idx.items(), key=lambda kv: kv[1],
                                  reverse=True)[:20])
            # 索引的键是路径，绝不能被当成配置做中英键转换
            self._write_json_atomic(path, idx, _display_keys=False)
            return True
        except Exception as e:
            self._log(f"记录备份目录索引失败（不影响主配置）: {e}", level='debug')
            return False

    def _remembered_backup_dirs(self):
        """读取缓存之外记下来的历史备份目录（按最近使用时间倒序）。"""
        path = _backup_dir_index_path()
        if not path or not os.path.isfile(path):
            return []
        try:
            idx = json.loads(open(path, 'r', encoding='utf-8').read())
        except Exception:
            return []
        if not isinstance(idx, dict):
            return []
        try:
            return [d for d, _ in sorted(idx.items(),
    ...
    def _config_backup_candidates(self):
        ...
        # [清缓存继承] 「配置备份目录」设到 APP 私有目录时，清数据会把目录
        # 连同这个设置项一起删掉，配置就彻底找不回来了。这里把缓存之外
        # 记下来的历史目录一并纳入候选，启动时就能自动捞回既往成果。
        for d in self._remembered_backup_dirs():
            add(os.path.join(d, CONFIG_BACKUP_NAME))

        # [废弃] 不再读取 APP 缓存目录 / 脚本目录 / 默认下载目录等历史遗留位置
        return out
    ...
    def _sanitize_recovered_config(self, data):
        ...
        for key in ('proxy', 'github_proxy'):
            ...
            if not (v.startswith('http://') or v.startswith('https://')):
                self._log(f"⚠️ 备份里的 {key} 不是合法地址（{v[:UI_TRUNC_URL]}），已忽略")
                data.pop(key, None)
        for key in ('download_output_dir', 'log_dir', 'config_backup_dir'):
            val = data.get(key)
            if val and isinstance(val, str) and not val.strip().startswith('/'):
                self._log(f"⚠️ 备份里的 {key} 不是绝对路径（{val[:UI_TRUNC_URL]}），已忽略")
                data.pop(key, None)
        return data

    def _config_backup_root(self):
        """备份存放根目录 = <数据根>/backups/（与数据根下的数据文件分开，
        这样"恢复默认"清数据根时不会把备份一起清掉）。"""
        try:
            return self._backups_root()
        except Exception:
            return os.path.join(DEFAULT_CONFIG_BACKUP_DIR, DATA_BACKUPS_SUBDIR)
    ...
    def _format_backup_time(self, stamp, mtime):
        ...
        if stamp:
            for fmt, ln in (("%y%m%d%H%M%S", 12), ("%y%m%d%H%M", 10),
                            ("%y%m%d%H", 8)):
                if len(stamp) == ln:
                    try:
    ...
    def _list_config_backups(self):
        """列出备份。一次备份 = <数据根>/backups/down_config[<时间戳>]/ 文件夹。

        [变更] 旧版一个备份就是一个 JSON 文件，装不下 map（接口新代码），
        恢复后编辑/跳转会指向缺失的文件，因此改成文件夹。
        """
        ...
        try:
            ...
            for fn in os.listdir(root):
                ...
                try:
                    st = os.stat(fp)
                    cfg = os.path.join(fp, CONFIG_BACKUP_NAME)
                    sites = 0
                    try:
    ...
    def _create_config_backup(self):
        ...
        while os.path.exists(path):
            n += 1
            path = os.path.join(root, "{}-{}".format(base, n))
        try:
            os.makedirs(path, exist_ok=True)
            data = self._snapshot_config(include_runtime=True)
            with self._config_io_lock:
                self._write_json_atomic(
                    os.path.join(path, CONFIG_BACKUP_NAME), data)
            # 接口新代码一起打包，恢复后才能直接编辑/跳转
            try:
                src_map = os.path.join(self._data_root(), CONFIG_BACKUP_SUBDIR)
                if os.path.isdir(src_map):
                    shutil.copytree(src_map, os.path.join(path,
                                                          CONFIG_BACKUP_SUBDIR),
                                    dirs_exist_ok=True)
            except Exception as e:
                self._log(f"备份 map 失败（配置已备份）: {e}", level='debug')
            return path, None
        except Exception as e:
            return None, "写入备份失败: {}".format(e)

    def _restore_backup_map(self, backup_dir):
        """把备份里的 map/ 整体还原到数据根（做到"恢复到备份前"）。"""
        src = os.path.join(str(backup_dir or ""), CONFIG_BACKUP_SUBDIR)
        if not os.path.isdir(src):
            return 0
        dst = os.path.join(self._data_root(), CONFIG_BACKUP_SUBDIR)
        try:
            ...
            self._log(f"恢复 map 失败（配置已恢复）: {e}", level='warn')
            return 0

    def _restore_config_backup_file(self, path):
        """恢复备份。

        path 可以是备份文件夹（新格式，含配置 + map），
        也可以是单个 JSON 文件（手工导出的配置）。
        """
        try:
            if os.path.isdir(str(path)):
                cfg_path = os.path.join(str(path), CONFIG_BACKUP_NAME)
            else:
                cfg_path = str(path)
    ...
    def _open_config_backup_dialog(self):
        ...
        def on_ui(act):
            ...
            def render():
                ...
                box = kit.vbox()
                box.setLayoutParams(kit.lp(-1, -2))
                box.addView(kit.hint("备份目录：{}".format(root)), kit.lp(-1, -2))
                box.addView(kit.hint("一份备份 = 一个文件夹（配置 + 接口新代码），"
                                     "恢复时整体还原。"),
            ...
            def do_import():
                ...
                start = self._config_backup_root() or \
                ...
                def on_picked(fp):
                    ...
                    self._log("✅ 已从备份恢复配置: {}".format(path))
                    refresh()

                # [备份改为文件夹] 现在一份备份是一个文件夹，改选目录
                self._show_file_browser(
                    "选取备份文件夹", start, mode="dir",
                    on_pick=on_picked,
                    manual_title="选取备份文件夹",
                    placeholder="输入备份文件夹完整路径")

            def do_pick_dir():

                start = self._data_root() or \
                    getattr(self, "download_output_dir", "") or \
                    "/storage/emulated/0"
                ...
                def on_picked(d):
                    ...
                    try:
                        ...
                        pass
                    kit.toast("数据目录已改为：{}".format(d), long=True)
                    self._log("✅ 数据目录（配置备份目录）已改为 {}".format(d))
                    refresh()

                self._show_file_browser(
                    "选取数据存放目录", start, mode="dir",
                    on_pick=on_picked,
                    manual_title="选取数据存放目录",
    ...
    def _open_backup_item_dialog(self, b, refresh):
        ...
        def on_ui(act):
            ...
            def do_rename():
                ...
                self._show_modern_input("改名备份", "输入新的备份名",
                                        b['name'], save)

            def do_delete():
                def confirm():
                    try:
                        # [备份改为文件夹] 整份删除（含里面的 map）
    ...
    def _mirror_config_to_backup(self, data, force=False, background=False):
        ...
        [性能] 镜像一次约 400ms（要读写整份主配置 persistent_config.json）。
    ...
    def _do_mirror_config_to_backup(self, data, force=False):
        ...
        # 以中文键名的形式同步进「配置备份目录」下的主配置文件
        # （persistent_config.json，早期版本叫 down_config.json），
        ...
        main_cfg = self._persistent_config_path()
        for d in self._config_backup_dirs():
            path = os.path.join(d, CONFIG_BACKUP_NAME)
            try:
                # [统一存储] 数据根里的那一份就是主配置，_save_persistent_config
                # 已经写过，这里不再重复写同一个文件（内容完全一样）
                if os.path.abspath(path) == os.path.abspath(main_cfg):
                    continue
                os.makedirs(d, exist_ok=True)
        ...
        if wrote > 0:
            # 写成功了才记索引：确保索引里的目录都真的存在过配置
            self._remember_backup_dirs()
        return wrote > 0

    def _load_persistent_config(self):

        _p = self._persistent_config_path()
    ...
    # [废弃] _legacy_config_locations / _purge_legacy_configs 已删除：
    # 旧版会去 APP 缓存目录、脚本目录、默认下载目录等一堆历史位置扫残留配置。
    # 现在数据只在唯一的数据根里，那些位置既不再写入、也不再读取，无需清理。

    def _purge_data_root(self):
        """清掉数据根里除 backups/ 之外的全部数据（「恢复默认设置」用）。

        白名单删除：只删本脚本自己写过的东西（主配置 / map / cache），
        用户在数据根里放的其它文件一律不动。
        """
        removed = []
        ...
        for nm in (CONFIG_BACKUP_NAME, "down_config.json", "config.json"):
            ...
            if os.path.isfile(fp):
                try:
                    ...
                    pass
        # 2) 数据子目录：map / cache / temp（backups 保留，备份不跟着清）
        for nm in (CONFIG_BACKUP_SUBDIR, DATA_CACHE_SUBDIR, "temp"):
            tp = os.path.join(root, nm)
            if os.path.isdir(tp):
                try:
    ...
    def _restore_default_config(self):
        try:
            # [统一存储] 清掉数据根里除备份之外的所有数据
            removed = self._purge_data_root()
            if removed:
                self._log("已清除数据目录下的数据 {} 项（备份已保留）".format(
                    len(removed)))
                for p in removed:
    ...
    def _decrypt_single_site(self, site_id):
        ...
        def _worker():
            try:
                    ...
                    if dec:
                        ...
                        if parse_success:
                            try:
                                ...
                                self._site_states[site_id]['decrypt_status'] = 'error'
                                self._site_states[site_id]['decrypt_msg'] = f'处理异常: {str(e)[:UI_TRUNC_SHORT]}'
                        else:
                ...
                self._site_states[site_id]['decrypt_status'] = 'error'
                self._site_states[site_id]['decrypt_msg'] = f'异常: {str(e)[:UI_TRUNC_SHORT]}'
                self._log(f"【解密】异常: {e}")
            finally:
                with self._site_op_lock:
    ...
    def _localize_single_site(self, site_id):
        ...
        def _worker():
            try:
                ...
                self._site_states[site_id]['localize_msg'] = f'失败: {str(e)[:UI_TRUNC_SHORT]}'
    ...
    def _is_local_file_site(self, url):
        """本地文件类接口：file:// 或 本机文件服务地址。

        [路径格式统一] 原来只认 file://。把「本地路径格式」设成「本机文件服务」
        之后生成的地址 http://127.0.0.1:<端口>/file/... 会被误判成远程接口，
        于是程序会对本地文件执行解密 / 本地化。这里两种写法都认。
        """
        s = str(url or "").strip()
        if not s:
            return False
        ...
        if not low.startswith(("http://", "https://")):
            return False
        return bool(self._local_url_to_path(s))
    ...
    def _update_package_download_site(self, old_id, new_name, new_url):
        """原子地改名 / 改地址：先落地新值，成功后再清理旧条目。

        [修复] 之前「先加后删」和「先删后加」都会丢数据：
          · 先删后加 —— 添加失败时旧接口一起没了（旧 BUG-16b）；
          · 先加后删 —— 按 url 命中时会**原地**改名并同步把 id 换成新 id，
            调用方手里那个 site 字典的 id 也跟着变了，于是
            `any(x['id'] == site['id'])` 命中的正是刚改好名的条目本身，
            把用户刚保存的接口直接删掉 = "改名保存就消失"。
        这里统一由本方法判断：只有真的新建了一条（未原地命中）才删旧条目。
        """
        old_id = str(old_id or "")
        cur = None
        for s in self.package_download_sites:
            if str(s.get("id", "")) == old_id:
                cur = s
                break
        if cur is None:
            # 旧条目已不存在（可能被别处删了）：退化成纯新增，不再动其它条目
            return self._add_or_update_package_download_site(new_name, new_url)

        # 名称撞到别的对象时拒绝，避免把另一条接口的 url 覆盖掉
        clean_name = self._normalize_package_download_name(new_name)
        for s in self.package_download_sites:
            if s is not cur and str(s.get("name", "")).casefold() == clean_name.casefold():
                raise ValueError("备注名已被其它接口占用：%s" % clean_name)

        saved, created = self._add_or_update_package_download_site(new_name, new_url)
        if created:
            # 新建了一条（没原地命中）：把旧条目摘掉，并迁移站点状态
            self.package_download_sites = [
                x for x in self.package_download_sites
                if str(x.get("id", "")) != old_id]
            if old_id in self._site_states:
                new_id = str(saved.get("id") or "")
                if new_id and new_id not in self._site_states:
                    self._site_states[new_id] = self._site_states.pop(old_id)
                else:
                    self._site_states.pop(old_id, None)
    ...
    def _guess_category(self, url, field_key=None):
        if field_key in ('spider', 'jar'):
            return 'jar'
        # [修复] url 是 None / dict / list 时，原来的 .split() 会抛 AttributeError
        # 穿透整个本地化流程；这里按"认不出来就当普通资源"处理。
        ...
        def progress_wrapper(url, cat, base_url, field_key=None):
            ...
            with lock:
                ...
                if now - last_progress_time[0] > 1.0 or completed[0] == total:
                    ...
                    self._push_log(f"⏳ 总进度 {completed[0]}/{total} ({pct:.1f}%) | 当前: {os.path.basename(url)[:UI_TRUNC_SHORT]}")
        ...
        if failed_items:
            ...
            for item in failed_items[:20]:
                try:
                    ...
                    continue
                brief.append("{} ({})".format(str(u)[-UI_TRUNC_TAIL:], str(why)[:UI_TRUNC_URL]))
            if brief:
                info["失败清单"] = brief
        if extra:
            info.update(extra)
    ...
    def _looks_like_file_ref(self, value, field_key=None, allow_absolute=False):
        ...
        if not v:
            return False
        # allow_absolute 只有统计"回源引用"时才会打开：
        # 那里要找的就是"本来写成 http 却没被本地化"的引用。
        # 默认关闭，保持原有的下载判定行为不变。
        if not allow_absolute and \
                v.lower().startswith(('http://', 'https://', 'file://', 'proxy://')):
            return False
        if field_key and str(field_key).lower() in self._NON_FILE_KEYS:
            return False

        # 绝对地址天然带 "?" 查询串，会命中下面的正则被误判成通配符，
        # 所以统计回源引用时先看 netloc + path 这一段。
        _rx_src = v
        _is_abs = v.lower().startswith(('http://', 'https://'))
        if allow_absolute and _is_abs:
            try:
                _sp = urllib.parse.urlsplit(v)
                _rx_src = (_sp.netloc or "") + (_sp.path or "")
            except Exception:
                _rx_src = v
        if re.search(r'[\[\]()*+?|^$\\{}]', _rx_src):
            return False
        path_part = v.split('?')[0].split(';')[0].rstrip('/')
        ext = os.path.splitext(path_part)[1].lower()
        has_dl_ext = bool(ext and ext in DOWNLOAD_EXTS)
        if v.startswith(('./', '../', '/')) or (allow_absolute and _is_abs):
    ...
    def _generate_local_box(self, data, source_name, output_dir, downloader,
        ...
        def localize_str(v, field_key):
            ...
            if local_path:
                return local_path
            s2 = v.strip()
            # [修复] 原来只有"不是 http 开头"的相对引用才会被处理，于是配置里
            # 本来就是绝对 http 地址、又没被本地化的引用（真正的回源引用）
            # 永远进不了统计，体检里的「回源引用数」恒为 0、查不出问题。
            # 这里补一段：绝对地址也计数，但指向本机架设服务的不算。
            if s2 and s2.lower().startswith(('http://', 'https://')):
                try:
                    if not downloader._is_own_service_url(s2):
                        _abs_dl = bool(self._downloadable_ext(s2)) and \
                            self._looks_like_file_ref(s2, field_key,
                                                      allow_absolute=True)
                        _abs_api = (not _abs_dl) and self._is_runtime_api_ref(
                            s2, field_key)
                        if _abs_dl or _abs_api:
                            remote_refs["n"] += 1
                            remote_refs["items"].append(s2)
                except Exception:
                    pass
                return v
            if s2 and base_url:
    ...
    # [统一存储] 备份文件夹（backups）也住进数据根，不能当成接口包去扫描
    ...
    # [要求] 分类前不再带 emoji 图标（统一以纯文字展示）
    FRAG_CAT_ICON = {}
    # 片段编辑缓存目录（编辑弹窗需要落盘才能复用现有文本编辑器）
    FRAG_EDIT_DIRNAME = "frag_edit"

    def _work_subdir_base(self):
        """工作根目录（map 等中间产物的父目录）= 数据根。

        [统一存储] 旧版在未设置 config_backup_dir 时回落到 download_output_dir，
        于是 map 会同时散在本地包目录和脚本目录两处；现在一律是数据根。
        """
        return self._data_root()
    ...
    def _scan_exclude_dirs(self):
        ...
        out = set()
        # [统一存储] 精确排除数据根下的三个子目录，而不是整个数据根：
        # 用户可能把数据根直接设在本地包目录里，整目录排除会导致扫不到任何接口
        _root = self._data_root()
        cand = [
            os.path.join(_root, CONFIG_BACKUP_SUBDIR),
            os.path.join(_root, DATA_CACHE_SUBDIR),
            os.path.join(_root, DATA_BACKUPS_SUBDIR),
            ...
            for idx, site in enumerate(sites, 1):
                ...
                try:
                    ...
                    try:
                        ...
                        self._site_states[sid]['localize_msg'] = f'失败: {str(e)[:UI_TRUNC_SHORT]}'
                    except Exception:
                        pass
                    self._clear_site_op(sid)
    ...
    def _show_big_input_dialog(self, act, title, code_hint="", name_hint=None,
        ...
        # [要求] 正文定高、自己滚动 → 名称框与底部按钮永远不会被顶出屏幕
        # [修复] min_height 要的是 **dp**：kit.input 内部会再乘一次密度，
        # 以前把已经换算成像素的 body_h 传进去，密度被乘了两次（密度 2.75 时
        # 0.46 屏高 ≈ 331dp → 910px → 再转一次 ≈ 2500px），编辑框被撑到屏幕外，
        # 外层又关掉了整体滚动，结果按钮看不见、框内也不滚、滚动条更不会出现。
        body_h = self._ui_text_body_height(kit, EDIT_MIN_HEIGHT_RATIO)
        body_dp = self._ui_text_body_height_dp(kit, EDIT_MIN_HEIGHT_RATIO)
        code_edit = kit.input(hint=code_hint, value=value, multiline=True,
                              mono=True, min_lines=lines,
                              min_height=(int(body_dp) or None))
        try:
            code_edit.setLayoutParams(kit.lp(-1, body_h))
        except Exception:
            pass
        box.addView(code_edit, kit.lp(-1, body_h))

        self._show_dialog(act, title, box, buttons,
                          height_ratio=height_ratio, scroll=False)
                ...
                try:
                    ...
                    for name, url in entries:
                        ...
                        if not name:
                            skipped += 1
                            if len(skip_msgs) < 3:
                                skip_msgs.append("缺少名称: {}".format(url[:UI_TRUNC_URL]))
                            continue
                        try:
                            with spider.lock:
                            ...
                            if status == 'dup_url':
                                skipped += 1
                                if len(skip_msgs) < 3:
                                    skip_msgs.append("地址重复: {}".format(url[:UI_TRUNC_URL]))
                            elif status == 'error':
                                failed += 1
                                if len(err_msgs) < 3:
                                    err_msgs.append("{}: {}".format(url[:UI_TRUNC_URL], why))
                            else:
                                added += 1
                        except Exception as exc:
                            failed += 1
                            if len(err_msgs) < 3:
                                err_msgs.append("{}: {}".format(url[:UI_TRUNC_URL], exc))
        ...
        def on_ui(act):
            ...
            def do_save():
                try:
                    ...
                    out_dir0 = spider.download_output_dir or os.path.join(spider._data_root(), "本地包")
                    ...
                    out_dir = spider.download_output_dir or os.path.join(spider._data_root(), "本地包")
                    ...
                    local_url = spider._local_url(file_path)
    ...
    def _ui_package_site_card(self, kit, site, index, refresh_fn):
        ...
        def on_edit():
            ...
            def on_save(values):
                ...
                try:
                    with spider.lock:
                        # [修复] 交给 _update_package_download_site 统一处理：
                        # 由它判断是"原地改名"还是"新建+删旧"，
                        # 不再在调用方按 id 二次删除（那样会把刚改名的条目删掉）。
                        spider._update_package_download_site(
                            site.get("id"), new_name, new_url)
                    kit.toast("已更新")
                    refresh_fn()
                except Exception as exc:
                    kit.toast("保存失败: {}".format(exc), long=True)
    ...
    def _open_root_dirs_management(self):
        ...
        def on_ui(act):
            ...
            add_card.addView(add_row,
                             kit.lp(-1, -2, 0.0, (0.0, UITheme.S_SM, 0.0, 0.0)))

            # [要求] 列表区定高滚动：目录再多，底部按钮（扫描接口 / 关闭）也不会被顶出屏幕
            dir_scroller, dirs_container = self._ui_fixed_scroll(kit, rows=4, ratio=0.20)
            switches = {}
            ...
            def refresh_dir_list():
                ...
                if not spider.root_dirs:
                    dirs_container.addView(kit.empty("还没有设置目录，先在上方添加一个"),
                                           kit.lp(-1, -2))
                    kit.fill_spring(dirs_container, 0)
                    return
            ...
            def do_pick_dir():
                # [要求] 智能定位：直接落在输入框里已填的有效路径上，避免从根目录逐层点进去
                start = spider._fs_start_dir(
                    str(edit.getText() or ""),
                    [spider.download_output_dir] + list(spider.root_dirs or []),
                    mode="dir")
                self._show_file_browser(
                    "选取设置目录", start, mode="dir",
                    on_pick=lambda v: (edit.setText(str(v)),
                                       kit.toast("已选择：{}".format(v))),
                    manual_title="选取设置目录",
                )

            # [要求] 与「本地爬虫程序管理」统一：输入框右侧直接放 选取 / 添加
            try:
                btn_pick.setOnClickListener(kit._on_click(do_pick_dir, None))
                btn_add.setOnClickListener(kit._on_click(do_add, None))
                kit.apply_focus(btn_pick, True)
                kit.apply_focus(btn_add, True)
            except Exception:
                pass
            ...
            list_card.addView(kit.section_title("📋 设置目录列表", hint="勾选参与扫描，长按可复制路径"),
                              kit.lp(-1, -2))
            list_card.addView(dir_scroller, kit.lp(-1, -2))
            refresh_dir_list()
            container.addView(list_card, kit.lp(-1, -2))
                    ...
                    def make_edit(nm=name, dd=_dir):
                        def on_edit():
                            ...
                            spider._show_modern_text_editor(
                                ...
                                "", spider._local_url(entry), on_saved)
                        return on_edit
    ...
    def _open_scan_dir_edit_dialog(self, path, on_done):
        ...
        def on_ui(act):
            ...
            # [修复] _ui_chip_row 用的是字符串取值（"default" / "custom"），
            # 这里原先塞的是 bool，导致：
            #   · 已有自定义类型的目录打开后芯片一个都不高亮；
            #   · 不动芯片直接点「保存」，do_ok 里 `state["custom"] == "custom"`
            #     恒为 False，于是 de.pop(path) 把该目录的自定义类型**悄悄清掉**
            #     —— 表现就是"改了参数不保存"。
            custom = "custom" if cur_exts else "default"
            state = {"custom": custom,
    ...
    def _open_scan_local_files_dialog(self):
        ...
        def on_ui(act):
            ...
            # ============ 3. 扫描目录 ============
            # [要求] 与「接口目录管理」统一风格：输入框 + 选取 + 添加，
            # 去掉下方的搜索框（目录就那么几个，搜索只会把添加区挤下去）。
            dir_card = kit.card()
            dir_card.addView(
                kit.section_title("📂 扫描文件夹",
                                  hint="输入或选取路径后点「添加」，勾选参与扫描"),
                kit.lp(-1, -2))
            ...
            # [修复] 目录列表定高滚动：列表再长也不会把「添加 / 扫描并生成」
            # 这些操作按钮顶出屏幕（原先按钮会随列表一起滚走）
            dir_scroller, dir_wrap = self._ui_fixed_scroll(kit, rows=4, ratio=0.22)
            dir_wrap.addView(dirs_container, kit.lp(-1, -2))
            dir_card.addView(dir_scroller,
                             kit.lp(-1, -2, 0.0, (0.0, UITheme.S_SM, 0.0, 0.0)))
            ...
            def do_pick_dir():
                ...
                # 输入框是完整目录 → 直接进该目录；
                # 是"还没建的路径" → 退到它最近的存在父目录，
                # 避免每次都从根目录一层层点进去。
                start = self._fs_start_dir(
                    str(dir_edit.getText() or ""),
                    [spider.download_output_dir] + list(spider.scan_local_dirs or []))
                self._show_file_browser(
    ...
    def _is_box_json(self, data, strict=False):
        """判断一份 JSON 是不是可用的接口 box 配置。

        基础判定（子目录沿用原标准）：顶层是 dict，且 sites 是 list。
        strict —— 根目录自判时使用，额外要求确实有可用站点：
        sites 为空、或每个站点都没有 key/name 的，一律算无效，
        避免把随手一个 {"sites": []} 或八竿子打不着的配置文件当成接口包收录。
        """
        if not isinstance(data, dict):
            return False
        sites = data.get("sites")
        ...
        for s in sites:
            if not isinstance(s, dict):
                continue
            if str(s.get("key") or "").strip() or str(s.get("name") or "").strip():
                return True
        return False

    def _collect_box_jsons(self, pkg_dir, map_dir, strict=False):
        """把 pkg_dir 下所有**有效**的 box json 绝对路径化后写进 map_dir。

        返回生成的 map 文件路径列表；一个有效文件都没有时返回空列表。
        [有效性检测] 读不出来 / 不是 dict / 没有 sites 列表的文件一律跳过；
        严格模式下还要求站点可用，防止无关 json 被当成接口。
        """
        out = []
        try:
            names = sorted(os.listdir(pkg_dir))
        except Exception as e:
            self._log("读取目录 {} 失败: {}".format(pkg_dir, e))
            return out
        for f in names:
            ...
            try:
                ...
                self._log("文件 {} 解析失败: {}".format(full_path, e))
                continue
            if not self._is_box_json(data, strict):
                self._log("跳过非接口文件 {}（缺少 sites 字段）".format(f),
                          level='debug')
                continue
            try:
                abs_data = self._absolutize_local_paths(data, pkg_dir)
                os.makedirs(map_dir, exist_ok=True)
                abs_json_path = os.path.join(map_dir, f)
                with open(abs_json_path, 'w', encoding='utf-8') as fp:
                    json.dump(abs_data, fp, ensure_ascii=False, indent=2)
                out.append(abs_json_path)
                self._log("生成绝对路径 JSON: {}".format(abs_json_path),
                          level='debug')
            except Exception as e:
                self._log("生成 map 失败 {}: {}".format(full_path, e))
        return out

    def _interface_map_base(self):
        """map 目录：<数据根>/map —— 扫描生成的"绝对路径化"接口副本都放在这里。"""
        return os.path.join(self._work_subdir_base(), "map")

    def _purge_interface_map(self, item):
        """删除一条接口记录时，连它 map 下的副本一起清掉（方案 1）。

        [为什么] map 是扫描时由源本地包生成的派生副本。只把记录从列表里摘掉、
        留着 map，下次扫描同一目录又会原样收录回来 —— 删除等于没删。

        [删什么] json_files 里的副本，以及空掉之后那个 map 包名目录。

        [不删什么] 用户的源本地包目录（设置目录 / 下载输出目录）一律不碰。
        [安全] 所有删除都先校验目标在 map 目录之内；路径解析不出、或不在 map
        目录下的，记一条 warn 后跳过，绝不误伤源目录。返回实际删除的文件数。
        """
        try:
            map_base = os.path.abspath(self._interface_map_base())
        except Exception:
            return 0
        killed = 0
        try:
        ...
        for p in files:
            ...
            if not ap:
                continue
            try:
                if os.path.commonpath([ap, map_base]) != map_base:
                    self._log("[接口删除] 跳过非 map 文件: %s" % ap, level='warn')
                    continue
            except Exception:
                continue
            try:
                dirs.add(os.path.dirname(ap))
            except Exception:
                pass
            try:
                ...
                self._log("[接口删除] 删除 map 文件失败 %s: %s" % (ap, e))

        # 副本清空的包名目录一并移除，不留孤儿（非空则说明还有别人在用，跳过）
        for d in dirs:
            try:
                ad = os.path.abspath(d)
                if (ad != map_base
                        and os.path.commonpath([ad, map_base]) == map_base
                        and os.path.isdir(ad) and not os.listdir(ad)):
                    os.rmdir(ad)
            except Exception:
                pass
        return killed
    ...
    def _rescan_localized_interfaces(self, selected_roots=None):
        ...
        [支持：根目录即本地包] 以前只把"设置目录下的子目录"当成一个个本地包，
        于是直接把本地包根目录（里面直接就是 xxx.json + js/）添加进来的小伙伴，
        扫出来是 0 个接口。现在除了子目录，还会检测设置目录自身：
        它下面的 json 若通过有效性检测，就把这个根目录当成一个本地包，
        包名 = 根目录名，挂在它的父目录下。

        [增量收录] 只有"本次扫描范围"内的条目会被替换，范围外的一律原样保留：
        先扫 A 再扫 B，A 的记录不会被 B 的结果冲掉；只有再次扫描 A 才会更新它。
        真正能改变一条记录的只有三种操作：同目录重扫（更新 map 数据）、
        管理本地接口里删除、以及隐藏（不动 map 数据）。
        """
        ...
        # 本次扫描覆盖的范围：每个 root 自身（它可能就是一个包）+ 它的直接子目录包
        scope_roots = set()
        for r in (selected_roots or []):
            try:
                scope_roots.add(os.path.normpath(str(r)))
            except Exception:
                pass
        ...
        def _in_scope(item):
            ...
            try:
                pkg = os.path.normpath(os.path.join(
                    str(item.get("parent_dir") or ""),
                    str(item.get("dir_name") or "")))
            except Exception:
                return False
            if pkg in scope_roots:                    # 根目录本身就是包
                return True
            return os.path.dirname(pkg) in scope_roots  # root 下的子目录包

        # 范围外的记录本次不动，原样保留（扫描 B 不会冲掉 A）
        kept = [it for it in (self.localized_interfaces or [])
                if not _in_scope(it)]

        existing_map = {}
        for item in (self.localized_interfaces or []):
        ...
        def _upsert(parent_dir, dir_name, json_files):
            """收录一个包：已存在就合并 json_files 并保留原 selected / hidden。"""
            if not json_files:
                return
            key = (parent_dir, dir_name)
        ...
        # [map 目录分配] 重名包（不同设置目录下的同名子目录、或根目录名与子目录名
        # 撞车）会写进同一个 map 目录，同名 json 互相覆盖。这里让后出现的重名让位：
        # 首次出现不加后缀（历史 map 路径不变），重名才追加 _2 / _3。
        # 作用域覆盖本次扫描的全部设置目录，跨目录撞车也能避开。
        used_map_names = set()
        # 范围外保留下来的记录仍占着它们原来的 map 目录，这里先登记，
        # 否则本次扫描可能分配到同名目录、把别人的 map 数据覆盖掉。
        for _it in kept:
            try:
                _p = _it.get("selected") or (_it.get("json_files") or [None])[0]
                if _p:
                    used_map_names.add(
                        os.path.basename(os.path.dirname(str(_p))).lower())
            except Exception:
                continue

        def _alloc_map_dir(name):
            base = str(name or "").strip() or "pkg"
            cand = base
            i = 1
            while cand.lower() in used_map_names:
                i += 1
                cand = "{}_{}".format(base, i)
            used_map_names.add(cand.lower())
            return os.path.join(map_base, cand)
        ...
        for root in selected_roots:
            ...
            for sub in subs:
                ...
                try:
                    ...
                    pass
                try:
                    files = self._collect_box_jsons(sub_path, _alloc_map_dir(sub))
                except Exception as e:
                    self._log(f"扫描子目录 {sub_path} 失败: {e}")
                    continue
                _upsert(root, sub, files)

            # [根目录即本地包] 检测设置目录自身：下面的 json 通过有效性检测才算一个包
            try:
                root_norm = os.path.normpath(root)
                root_name = os.path.basename(root_norm) or ""
                if root_name:
                    root_files = self._collect_box_jsons(
                        root_norm, _alloc_map_dir(root_name), strict=True)
                    if root_files:
                        _upsert(os.path.dirname(root_norm), root_name, root_files)
                        self._log("[根目录包] {} 本身即本地包，收录 {} 个配置".format(
                            root_norm, len(root_files)))
            except Exception as e:
                self._log(f"检测根目录自身失败 {root}: {e}")

        # [增量收录] 本次结果在前（刚扫的立刻可见），范围外的老记录原样接在后面
        self.localized_interfaces = new_items + kept
        self._save_config_to_file()
        _msg = "扫描完成：本次 {} 个".format(len(new_items))
        if kept:
            _msg += "，另有 {} 个不在本次范围内已保留".format(len(kept))
        self._notify_app(_msg)
        self._log("扫描本地接口：本次 {} 条，保留 {} 条，合计 {} 条".format(
            len(new_items), len(kept), len(self.localized_interfaces)))
    ...
    def _switch_to_oktv(self, box_path, parent_dir, dir_name):
        try:
            ...
            if self.inject_manager_site and SELF_PATH:
                # [按 box 写法] 绝对写法原样、相对写法按 box 目录拼，与 api 同一套规则
                _cfg_url = self._manager_config_file_url()
                manager_site = {
                    ...
                    "api": self._manager_script_url(_cfg_url, original_data.get("sites")),
            ...
            sync_payload = {
                "type": 0,
                "url": self._local_url(config_path),
                "name": f"本地化[{dir_name}]"
            }
    ...
    @staticmethod
    def _frag_body_name(body):
        """代码里真实写着的名称；没有 name / title 时返回空串（区别于 _frag_name 的兜底值）。

        用于「编辑代码后回写列表显示名」——拿不到真名时不能拿兜底的
        "片段N" 去覆盖用户自定义的名字。
        """
        if isinstance(body, dict):
            for k in ("name", "title"):
    ...
    def _ui_text_body_height_dp(self, kit, ratio=EDIT_MIN_HEIGHT_RATIO,
                                min_dp=140.0):
        """大文本编辑区的定高 **dp 值**（给需要 dp 的 Android API 用）。

        注意 Android 的两类高度 API 单位不同：
          · setMinHeight / setHeight 传的是像素，kit 内部会再乘一次密度；
          · LayoutParams 的 height 直接用像素原值。
        以前只提供像素版，调用方把像素塞给 setMinHeight，密度被乘了两次，
        编辑框被撑到屏幕高度的几倍，外层又不让滚，于是既看不到底部按钮、
        框内也不会滚、更不会出现滚动条。这里把 dp 值和像素值分开提供。
        """
        try:
            h = float(getattr(kit, "h_dp", 0) or 0) * float(ratio)
        except Exception:
            h = 0.0
        if h <= 0:
            h = 320.0
    ...
    def _ui_text_body_height(self, kit, ratio=EDIT_MIN_HEIGHT_RATIO, min_dp=140.0):
        """大文本编辑区的定高 **像素值**：给 LayoutParams 用。

        [要求] 按钮随文本框一起滚动时，滚到下面就点不到按钮了；
        这里按屏幕高度给正文一个固定高度，正文自己在框内滚动。
        """
        return kit.dp(int(self._ui_text_body_height_dp(kit, ratio, min_dp)))
    ...
    def _open_favorite_manage_dialog(self):
        ...
        def on_ui(act):
            kit = self._kit(act)
            holder = {}
            # page: 分页渲染，避免上千个片段一次建卡把主线程卡死
            state = {"cat": "all", "kw": "", "page": 1}
            sw_refs = {}
            # [修复] 纯 Python 侧的「片段 → 开关/标题」登记表。
            # 之前只把引用挂在 Java View 的属性上（card._switch），
            # 赋值在部分机型上会静默失败，导致全选/反选/清空后开关形态不变，
            # 但后台数据其实已经改了。这里以 Python 登记表为准，view 树兜底。
            card_refs = []
            ...
            try:
                more_btn.setOnClickListener(kit._on_click(load_more, None))
                kit.apply_focus(more_btn, True)
            except Exception:
                pass
            ...
            def _sync_switches(frags):
                ...
                现在改为静默设置（临时摘除 listener）+ Python 登记表 / view 树兜底。
                """
                wanted = set(id(f) for f in frags)
                for rec in list(card_refs):
                    f = rec.get("frag")
                    if f is None or id(f) not in wanted:
                        continue
                    on = bool(f.get("enabled", True))
                    sw = rec.get("sw")
                    if sw is None:
                        card = sw_refs.get(id(f)) or _find_card(f)
                        sw = getattr(card, "_switch", None) if card is not None else None
                    if sw is not None:
                        try:
                    ...
                    if tv is None:
                        card = sw_refs.get(id(f)) or _find_card(f)
                        tv = getattr(card, "_title_view", None) if card is not None else None
            ...
            def make_card(frag):
                ...
                def do_rename():
                    def _save(v):
                        ...
                        if v:
                            frag["name"] = v
                            # [修复] 改名必须同步进片段代码：
                            # 生成接口时输出的是 body，只改 frag["name"]
                            # 会出现"列表显示新名、生成的代码还是旧名"。
                            body = frag.get("body")
                            if isinstance(body, dict):
                                for k in ("name", "title"):
                                    if k in body:
                ...
                def do_edit():
                    # 编辑回来要刷新列表，否则界面还停在旧名称 / 旧分类
                    self._fav_edit_fragment(
                        frag, lambda: (refresh_count(), refresh_list()))

                def do_proxy():
                    self._fav_ask_proxy(
                        frag,
                        on_ok=lambda v: (self._fav_save(),
                                         refresh_count(),
                                         refresh_list()))
                ...
                actions = [
                    ...
                    # [要求] 移除按钮前增加「注代」：给片段代码注入代理参数
                    {"text": "注代", "style": "soft_brand", "callback": do_proxy, "dismiss": False},
                ...
                card = self._ui_site_card(
                    ...
                    dim=not frag.get("enabled", True), per_row=6, refs=refs)
            ...
            def refresh_list():
                ...
                if not vis:
                    ...
                    kit.fill_spring(list_box, 0)
                    return
                # 分页：一次只建当前页的卡，剩下的点「加载更多」再建
                page_vis = vis[:state.get("page", 1) * self.FRAG_PAGE_SIZE]
                for frag in page_vis:
                    list_box.addView(make_card(frag),
                                     kit.lp(-1, -2, 0.0, (0.0, 0.0, 0.0, UITheme.S_XS)))
                if len(page_vis) < len(vis):
                    try:
                        more_btn.setText(
                            "加载更多（还有 {} 条）".format(len(vis) - len(page_vis)))
                        more_box.setVisibility(0)
                        list_box.addView(more_box, kit.lp(-1, -2))
                    except Exception:
                        pass
                kit.fill_spring(list_box, 0)
    ...
    def _open_frag_picker_dialog(self, info):
        ...
        def on_ui(act):
            ...
            sw_refs = {}
            # [修复] 同接口改装中心：用 Python 登记表保证批量操作能同步开关形态
            card_refs = []
            ...
            def refresh_list():
                ...
                for i, fr in page_vis:
                    ...
                    card = self._ui_site_card(
                        ...
                        dim=bool(fr.get("dup")), per_row=3, refs=refs)
                    sw_refs[i] = card
                    card_refs.append({
                        "idx": i, "sw": refs.get("switch"),
                        "title": refs.get("title"), "card": card})
                    try:
                        card._frag_idx = i
                    except Exception:
                        pass
            ...
            def _batch(mode):
                ...
                for i, fr in vis:
                    ...
                    n += 1
                want = set(i for i, fr in vis if not fr.get("dup"))
                for rec in list(card_refs):
                    i0 = rec.get("idx")
                    if i0 is None or i0 not in want:
                        continue
                    sw = rec.get("sw")
                    if sw is None:
                        card = sw_refs.get(i0) or _find_card(i0)
                        sw = getattr(card, "_switch", None) if card is not None else None
                    if sw is not None:
                        try:
                            kit.set_switch_quietly(
                                sw, bool(state["picked"].get(i0, False)))
                        except Exception:
                            pass
    ...
    def _fav_edit_fragment(self, frag, refresh_cb=None):
        ...
        try:
            # [统一存储] 片段编辑缓存落在数据根/cache 下，不再写脚本目录
            edit_dir = os.path.join(self._data_cache_dir(), self.FRAG_EDIT_DIRNAME)
            os.makedirs(edit_dir, exist_ok=True)
        except Exception:
            edit_dir = self._data_cache_dir()
        ...
        def on_saved():
            try:
                ...
                # [修复] 代码里改了 name / title 要回写到列表显示名，
                # 否则"编辑了 name，前端还显示旧名"。
                # 代码里没有可用名称时保留原名，不把它清空。
                _nm = self._frag_body_name(new_body)
                if _nm:
                    frag["name"] = _nm
                self._fav_save()
                self._fav_emit()
    ...
    def _fav_ask_proxy(self, frag, on_ok=None):
        """「注代」：给当前片段代码注入一个代理参数。

        [要求] 默认写 "proxy": "http://127.0.0.1:2080"，
        地址需要弹窗确认且可修改；改过之后记住新值，下次默认带出来。
        """
        body = frag.get("body")
        if not isinstance(body, dict):
            self._notify_app("该片段没有可写入的代码")
            return
        default = str(getattr(self, 'frag_proxy_default', '') or '').strip() \
            or self.DEFAULT_FRAG_PROXY
        try:
        ...
        def _ui(act):
            ...
            box.addView(kit.hint(
                "在当前片段代码里写入参数：\n    \"proxy\": \"<代理地址>\"\n"
                "留空并确定 = 移除该片段已有的 proxy 参数。"),
                kit.lp(-1, -2))
            edit = kit.input(hint="代理地址，如 http://127.0.0.1:2080",
                             value=default)
            ...
            def do_ok():
                ...
                if v:
                    ...
                    # 记住本次输入，下次「注代」直接带出来
                    try:
                        self.frag_proxy_default = v
                    except Exception:
                        pass
                else:
                    body.pop("proxy", None)
                try:
                    frag["api"] = self._frag_api_value(body)
                except Exception:
                    pass
                if on_ok:
                    on_ok(v)
            ...
            self._fav_dialog(act, "注代 - {}".format(frag.get("name") or ""),
                             box, buttons, holder, height_ratio=0)
            try:
                edit.requestFocus()
            except Exception:
                pass
    ...
    def _iface_source_files(self, info):
        """管理本地接口的「来源」候选：只取 map 下维护的那份数据。

        [要求] 编辑 / 视图 / 跳转的来源应当是 map 下的数据（不是解密或本地化
        原始文件）；同一目录下存在多份数据时全部列出，供「选择来源」切换。
        map 下还没有就现生成一份再返回。
        """
        out, seen = [], set()
        try:
        ...
        if not out:
            try:
                gen = self._ensure_generated_json(info)
                if gen and os.path.isfile(gen):
                    out.append(os.path.abspath(str(gen)))
            except Exception:
                pass
        try:
            out.sort()
    ...
    def _iface_source_of(self, info, edit_src=None, item_key=None):
        """当前生效的来源文件（优先用户手动指定 → selected → map 候选第一条）"""
        if edit_src is not None and item_key is not None:
            v = edit_src.get(item_key)
            if v and os.path.isfile(str(v)):
                return str(v)
        files = self._iface_source_files(info)
        if files:
            sel = str(info.get("selected") or "")
            if sel:
                ap = os.path.abspath(sel)
                for f in files:
                    if f == ap:
                        return f
            return files[0]
        return str(info.get("selected") or "")

    def _iface_write_back(self, info, data, target):
        """把改动写回「源文件」：编辑目标 +（若不同）真实源文件。

        [要求] 管理本地接口里的所有操作都保存至源文件，
        不生成新包、不写生成历史、不重新扫描。
        """
        wrote = []
        try:
            tgt = os.path.abspath(str(target or ""))
            if not tgt:
                return wrote
            ...
            with open(tgt, 'w', encoding='utf-8') as f:
                json.dump(data, f, ensure_ascii=False, indent=2)
            wrote.append(tgt)
            # 同步回写真实源文件（非 map 的那份），保证重新扫描后改动仍在
            try:
                real = self._real_source_json(info)
                if real:
                    rp = os.path.abspath(str(real))
                    if rp != tgt and os.path.isfile(rp):
            ...
            self._log(f"写回源文件失败: {e}", level='error')
        return wrote

    def _open_interface_frag_view(self, info, on_changed=None):
        """「视图」弹窗：形态参照「接口改装中心」一级弹窗。

        与接口改装中心的区别（[要求]）：
          · 所有操作直接保存至**源文件**（当前编辑目标 + 真实源文件），
            不生成新包、不写「生成历史」；
          · 底部按钮是「保存」而不是「生成并保存」，不触发重新生成 / 重新扫描。
        """
        dir_name = str(info.get("dir_name") or "未命名")
        target = self._iface_source_of(info)
        if not target or not os.path.isfile(target):
            self._notify_app("未找到可用的接口数据文件：{}".format(dir_name))
            return
        ...
        try:
            ...
            self._notify_app("读取失败: {}".format(e))
            return
        if not isinstance(data, dict):
            self._notify_app("接口数据顶层必须是 JSON 对象")
            return

        tail_keys = [(k, v) for k, v in data.items()
                     if k not in ("sites", "lives")]
        has_lives = "lives" in data

        def build_frags():
            out = []
            for grp in ("sites", "lives"):
                for b in (data.get(grp) or []):
                    if not isinstance(b, dict):
                        continue
                    out.append({
                        "group": grp,
                        "body": b,
                        "name": self._frag_name(b) or "未命名",
                        "api": self._frag_api_value(b),
                        "cat": self._frag_category(b, grp),
                        "enabled": True,
                    })
            return out
        ...
        def compose(only_enabled=True):
            """按当前顺序 / 勾选状态拼回完整 JSON"""
            new_data = {}
            for k, v in tail_keys:
                new_data[k] = v
            if has_lives:
                new_data["lives"] = [
                    f["body"] for f in frags
                    if f.get("group") == "lives"
                    and (f.get("enabled", True) or not only_enabled)]
            new_data["sites"] = [
                f["body"] for f in frags
                if f.get("group") != "lives"
                and (f.get("enabled", True) or not only_enabled)]
        ...
        def save_now(toast=True):
            wrote = self._iface_write_back(info, compose(True), target)
            if not wrote:
                self._notify_app("保存失败，未能写入源文件")
                return False
            if on_changed:
                try:
                    on_changed()
                    ...
                    pass
            if toast:
                self._notify_app("已保存至源文件\n{}".format(
                    os.path.basename(wrote[0])))
            return True
        ...
        def on_ui(act):
            ...
            head_card = kit.card(
                pad=(UITheme.S_MD, UITheme.S_XS, UITheme.S_MD, UITheme.S_XS))
            head_card.addView(
                kit.section_title("🧩 片段视图 - {}".format(dir_name),
                                  hint="勾选=保留；取消勾选后点「保存」即从源文件移除"),
                kit.lp(-1, -2))
            cat_entries = [(k, lbl) for k, lbl in self.FRAG_CATS]
            head_card.addView(
                self._ui_chip_row(kit, cat_entries, state, "cat",
            ...
            srow, _se = self._ui_search_box(
                kit, lambda t: (state.__setitem__("kw", t),
                                state.__setitem__("page", 1),
                                refresh_list(), refresh_count()),
                hint="搜索名称 / 地址")
            head_card.addView(srow,
                              kit.lp(-1, -2, 0.0, (0.0, UITheme.S_XS, 0.0, 0.0)))
            count_label = kit.text("", size=UITheme.FS_CAPTION,
                                   color=UITheme.TEXT_3, max_lines=2)
            head_card.addView(count_label,
                              kit.lp(-1, -2, 0.0, (0.0, UITheme.S_XXS, 0.0, 0.0)))
            container.addView(head_card, kit.lp(-1, -2))

            # ---- 定高片段列表（不随页面滚动，底部按钮永远可见）----
            list_card = kit.card()
            scroller, list_box = self._ui_fixed_scroll(kit, rows=6, ratio=0.34)
            list_card.addView(scroller, kit.lp(-1, -2))
            list_card.addView(
            ...
            def visible():
                ...
                out = []
                for f in frags:
                    if cat != "all" and f.get("cat") != cat:
                        continue
                    if kw:
                        hay = "{} {} {}".format(
                            f.get("name") or "", f.get("api") or "",
                            f.get("group") or "").lower()
                        if kw not in hay:
                            continue
                    out.append(f)
                return out
            ...
            def refresh_count():
                ...
                try:
                    count_label.setText(
                        "当前分类 {} 条，已选 {} 条 ｜ 全部 {} 条".format(
                            len(vis), sel, len(frags)))
                except Exception:
                    pass

            def _sync_cards(targets=None):
                """把开关形态同步到数据（全选 / 反选 / 清空后）"""
                for rec in list(card_refs):
                    try:
                        f, sw, tv = rec.get("frag"), rec.get("sw"), rec.get("title")
                    except Exception:
                        continue
                    if f is None:
                        continue
                    if targets is not None and f not in targets:
                        continue
                    on = bool(f.get("enabled", True))
                    try:
                        if sw is not None:
            ...
            def _batch_delete():
                ...
                def _yes():
                    kill = set(id(f) for f in vis)
                    frags[:] = [f for f in frags if id(f) not in kill]
                    save_now(toast=False)
                    refresh_list()
                    refresh_count()
                    kit.toast("已删除 {} 个片段并保存至源文件".format(len(vis)))

                self._show_modern_confirm(
                    "删除片段",
                    "确定从源文件移除勾选的 {} 个片段？\n（保存后不可撤销）".format(len(vis)),
                    _yes)

            def make_card(frag):
                cat = frag.get("cat", "other")
                sub = "[{}] {}".format(self.FRAG_CAT_LABEL.get(cat, cat),
                                       "直播" if frag.get("group") == "lives" else "点播")
                api = frag.get("api") or ""
                if api:
                    sub += "\n{}".format(_short_url(api, 44))
                ...
                def _idx_of():
                    """按对象身份定位（片段是 dict，不能靠 == 找，会撞到同内容的片段）"""
                    for _i, _f in enumerate(frags):
                        if _f is frag:
                            return _i
                    return -1
                ...
                def _re_sort(step):
                    ...
                    if i < 0:
                        return
                    j = i + step
                    if j < 0 or j >= len(frags):
                        kit.toast("已经在最{}面".format("前" if step < 0 else "后"))
                        return
                    ...
                    def _save(v):
                        ...
                        if isinstance(body, dict):
                            ...
                            for k in ("name", "title"):
                                ...
                                body["name"] = v
                        frag["api"] = self._frag_api_value(body or {})
                        save_now(toast=False)
                        refresh_list()
                        kit.toast("已改名并保存至源文件")
                    self._show_modern_input(
                        "改名片段", "输入新的片段名称",
                        frag.get("name") or "", _save)
                ...
                def do_edit():
                    def on_done():
                        body = frag.get("body") or {}
                        frag["api"] = self._frag_api_value(body)
                        frag["cat"] = self._frag_category(body, frag.get("group"))
                        nm = self._frag_body_name(body)
                        if nm:
                            frag["name"] = nm
                        save_now(toast=False)
                        refresh_list()
                ...
                def do_proxy():
                    self._fav_ask_proxy(
                        frag,
                        on_ok=lambda v: (save_now(toast=False),
                                         refresh_list()))
                ...
                def do_remove():
                    def _yes():
                        ...
                        refresh_list()
                        refresh_count()
                        kit.toast("已移除：{}".format(frag.get("name") or ""))
                    self._show_modern_confirm(
                        "移除片段",
                        "确定从源文件移除「{}」？".format(frag.get("name") or ""),
                        _yes)
            ...
            buttons = [
                {"text": "💾 保存", "style": "success",
                 "callback": lambda: (save_now() and kit.toast("已保存至源文件")),
                 "dismiss": False},
                {"text": "关闭", "style": "secondary", "callback": None, "dismiss": True},
            ]
            self._fav_dialog(act, "👁 视图 - {}".format(dir_name), container,
                             buttons, holder, height_ratio=0.92, scroll=False)
    ...
    def _open_manage_switch(self):
        ...
        def on_ui(act):
            ...
            def make_item_card(info):
                ...
                # [要求] 来源 = map 下的数据；有多份数据时全部列出供选择
                files = spider._iface_source_files(info)
                src_count = len(files)

                def cur_src():
                    c = spider._iface_source_of(info, edit_src, item_key)
                    return str(c)
                ...
                def on_pick_src():
                    ...
                    def _ok(v):
                        ...
                        try:
                            ...
                            pass
                        refresh_list()
                        kit.toast("来源已设为：{}".format(os.path.basename(str(v))))

                    spider._show_modern_radio_selector(
                        "选择来源 - {}".format(dir_name), options,
                        cur_src() or files[0], _ok)

                def on_edit():
                    # 编辑一律加载 map 下维护的那份数据
                    selected = cur_src() or spider._iface_source_of(info)
                    if selected and os.path.exists(selected):
                        try:
                            ...
                            spider._show_modern_text_editor(
                                f"✏️ 编辑代码 - {dir_name}", content, selected, "",
                                spider._local_url(selected),
                                on_saved,
                ...
                def on_view():
                    spider._open_interface_frag_view(
                        info, on_changed=lambda: refresh_list())

                def on_del():
                    _gone = spider._purge_interface_map(info)
                    ...
                    spider._notify_app(
                        f"已删除记录：{dir_name}"
                        + ("\n（同时清除 {} 个 map 副本，源本地包不动）".format(_gone)
                           if _gone else ""))
                ...
                actions = [
                    {"text": "代码", "style": "primary", "callback": on_edit,
                     "dismiss": False},
                    {"text": "视图", "style": "soft_brand", "callback": on_view,
                     "dismiss": False},
                ]
                def _confirm_del():
                    spider._show_modern_confirm(
                ...
                _sub = f"{full_path}"
                _sub += "\n来源：{}".format(
                    os.path.basename(str(_src_show)) or "未生成")
                if src_count > 1:
                    _sub += "（共 {} 份，可切换）".format(src_count)
                if hidden:
                    _sub += "\n（已隐藏）"
                card = spider._ui_site_card(
                    ...
                    _sub,
                    batch_selected.get(item_key, False), on_toggle, actions,
                    dim=hidden, per_row=4)
                return card

            def selected_infos():
                keys = [k for k, v in batch_selected.items() if v]
                if not keys:
                    return []
                return [(info, (info.get("parent_dir", ""), info["dir_name"]))
                        for info in spider.localized_interfaces
                        if (info.get("parent_dir", ""), info["dir_name"]) in keys]
            ...
            def batch_toggle_visibility():
                picked = selected_infos()
                if not picked:
                    kit.toast("请先勾选要批量操作的接口")
                    return
                show_count = 0
                hide_count = 0
                for info, _k in picked:
            ...
            def batch_delete():
                """[要求] 选中的一键删除：连同 map 副本一起清（源本地包不动）"""
                picked = selected_infos()
                if not picked:
                ...
                def _yes():
                    kill = set(k for _info, k in picked)
                    _gone = 0
                    for _info, _k in picked:
                        _gone += spider._purge_interface_map(_info)
                    spider.localized_interfaces = [
                        i for i in spider.localized_interfaces
                        if (i.get("parent_dir", ""), i["dir_name"]) not in kill]
                    for k in kill:
                        batch_selected.pop(k, None)
                        edit_src.pop(k, None)
                    spider._save_config_to_file()
                    refresh_list()
                    kit.toast("已删除 {} 条接口记录，清除 {} 个 map 副本".format(
                        len(kill), _gone), long=True)

                spider._show_modern_confirm(
                    "删除接口记录",
                    "确定删除选中的 {} 条记录？\n（同时清除 map 下的接口副本，\n"
                    "源本地包目录不动）".format(len(picked)), _yes)

            def do_clear_all():
                def _clear():
                    _gone = 0
                    for _it in (spider.localized_interfaces or []):
                        _gone += spider._purge_interface_map(_it)
                    spider.localized_interfaces = []
                    spider._save_config_to_file()
                    spider._notify_app("已清空所有接口记录"
                                       + ("，清除 {} 个 map 副本".format(_gone)
                                          if _gone else ""))
            ...
            list_card.addView(
                self._compact_button_bar(kit, [
                    {"text": "全选", "style": "secondary",
                     "callback": select_all_items, "dismiss": False},
                    {"text": "反选", "style": "secondary",
                     "callback": invert_items, "dismiss": False},
                    {"text": "清空", "style": "secondary",
                     "callback": clear_select, "dismiss": False},
                ], size="sm", per_row=3),
                kit.lp(-1, -2, 0.0, (0.0, UITheme.S_XS, 0.0, UITheme.S_XS)))
            ...
            list_card.addView(
                self._compact_button_bar(kit, [
                    ...
                    {"text": "批量显隐", "style": "soft_brand",
                     "callback": batch_toggle_visibility, "dismiss": False},
                ], size="sm", per_row=2),
                kit.lp(-1, -2, 0.0, (0.0, 0.0, 0.0, UITheme.S_SM)))

            # [要求] 列表定高滚动，操作按钮固定在列表上方不随列表滚走
            list_scroller, list_wrap = self._ui_fixed_scroll(kit, rows=5, ratio=0.34)
            list_wrap.addView(sites_container, kit.lp(-1, -2))
            list_card.addView(list_scroller, kit.lp(-1, -2))
            refresh_list()
            container.addView(list_card, kit.lp(-1, -2))
            ...
            self._show_dialog(act, "管理本地接口", container, buttons,
                              height_ratio=0.88, scroll=False)
        self._run_on_ui(on_ui)
    ...
    def _is_map_local_json(self, url):
        ...
        if not url or not isinstance(url, str):
            return False
        # [路径格式统一] file:// 与 http://127.0.0.1:<任意端口>/file/ 都算本地地址，
        # 原来只认 file://，于是用文件服务地址写进来的 map 会被误判成远程。
        path = self._local_url_to_path(url) or url
        if not os.path.isabs(path):
            return False
        map_dir = os.path.join(self._work_subdir_base(), "map")
        try:
    ...
    def init(self, extend=""):
        with self.lock:
            ...
            if ext:
                ...
                if config_file:
                    ...
                    if cf_config:
                        ...
                        # 记下本次外部文件实际给出的键值，用于下次启动判别
                        # "用户是否真的改过这个文件"（见下方持久化合并）
                        try:
                            ext_cfg_snapshot = copy.deepcopy(cf_config)
                        except Exception:
                            ext_cfg_snapshot = cf_config
                    else:
                        self._log(f"【init】❌ 外部配置加载失败: {config_file}")

                        _why = self._config_fail_reason(config_file)
                        self._log("【init】失败原因: %s" % _why, level='error')
            ...
            if persistent and isinstance(persistent, dict):
                ...
                # 上一次启动时外部配置文件给出的值（用于判断用户是否又手改过）
                prev_ext = persistent.get(self.EXT_APPLIED_KEY)
                prev_ext = dict(prev_ext) if isinstance(prev_ext, dict) else {}
                merged_count = 0
                skipped_ext = 0
                for k, v in persistent.items():
                    if k == self.EXT_APPLIED_KEY:
                        continue
                    if k == 'config_file':

                        if v and not self._config_file_path:
                            # [失效纠正] 持久化的可能是很久以前的路径，先确认还在
                            _p = self._strip_file_scheme(str(v))
                            try:
                                _ok = (self._is_remote_path(_p)
                                       or os.path.isfile(_p))
                            except Exception:
                                _ok = False
                            if _ok:
                                ...
                                self._log("【init】持久化的配置文件已失效，忽略: %s"
                                          % v, level='warn')
                        continue
                    # [FIX BUG-07 + 新增] 外部配置文件加载成功时，原则上"以外部为准"，
                    # 防止用户手编 down_config.json 后被旧持久化静默回滚。
                    #
                    # 但原判断 `k in self.config` 过于宽松：__init__ 已经把
                    # root_dirs / sources / scan_* 等键全部预置进 self.config，
                    # 于是只要外部文件存在，持久化的用户数据就**整片被跳过**——
                    # 表现就是"添加的接口 / 改过的参数重启后没了"。
                    #
                    # 现在改成逐键比对：只有当外部文件该键的值相对"上次启动时"
                    # 发生了变化（说明用户真的改过文件），才让外部值优先；
                    # 外部文件没动过的键，一律以持久化（用户后来在界面里改的）为准。
                    if ext_file_loaded and ext_cfg_snapshot is not None \
                            and k in ext_cfg_snapshot:
                        try:
                            unchanged = (k in prev_ext
                                         and prev_ext.get(k) == ext_cfg_snapshot.get(k))
                        except Exception:
                            unchanged = False
                        if not unchanged:
                            skipped_ext += 1
                ...
                # 记下本次外部文件的快照，供下次启动比对
                if ext_file_loaded and ext_cfg_snapshot is not None:
                    self.config[self.EXT_APPLIED_KEY] = ext_cfg_snapshot
                elif self.EXT_APPLIED_KEY in self.config and not ext_file_loaded:
                    self.config.pop(self.EXT_APPLIED_KEY, None)
                if skipped_ext:
                    self._log(f"【init】外部配置有 {skipped_ext} 项与上次不同，已按外部文件为准")
                self._apply_config(self.config)
            ...
            # [入口固化] 首次运行就把管理入口的脚本地址记下来；
            # 此刻若解析不出可靠地址（拿不到 box 原始数据等）会自动跳过，
            # 等生成产物 / 切换接口时拿到真实路径再补记。
            try:
                if not self._manager_self_path():
                    # 优先从当前 box 里直接读出 api 原值：api 与 config_file 跨目录、
                    # 跨域名（./py/ + ./json/、两个不同站点）时也认得出来。
                    _box = str(getattr(self, "_original_oktv_url", "")
                               or "").strip()
                    if _box:
                        _api = self._resolve_self_api_from_box(_box)
                        if _api:
                            self._freeze_manager_script(_api)
                self._manager_script_url()
            except Exception:
                pass
                    ...
                    def do_edit():
                        try:
                            with open(local_path, 'r', encoding='utf-8') as f:
                                content = f.read()
                            remote_url = site['url']
                            local_url = self._local_url(local_path)
                            def on_save():
    ...
    def _show_modern_text_editor(self, title, content, file_path, remote_url, local_url,
        ...
        def on_ui(act):
            ...
            # [要求] 顶部工具行（编辑/保存/跳转/复制）不随正文滚动：
            # 给正文定高、正文自己在框内滚动，整个对话框不再整体滚动。
            body_h = spider._ui_text_body_height(kit, 0.46)
            try:
                edit.setLayoutParams(kit.lp(-1, body_h))
            except Exception:
                pass
            box.addView(edit, kit.lp(-1, body_h))
    ...
    def _setting_remarks(self, spec):
        ...
        if fmt == "trunc_short":
            text = str(value or "")
            return (text[:UI_TRUNC_SHORT] + "...") \
                if len(text) > UI_TRUNC_SHORT else text
    ...
    def _notify_app(self, message, wait=False, replace=False):
        ...
        if not text or self._destroyed:
            return False
        if len(text) > UI_TRUNC_TOAST:
            text = text[:UI_TRUNC_TOAST - 3] + "..."
        try:
            ...
            class Show(dynamic_proxy(Runnable)):
                ...
                def run(self):
                    try:
                        toast = toast_class.makeText(act, text[:UI_TRUNC_TOAST_SHOWN],
                    ...
                    def make_edit(s=site):
                        ...
                        def on_edit():
                            def on_save(values):
                                ...
                                try:
                                    with spider.lock:
                                        # [修复] 同 _ui_package_site_card：
                                        # 由 _update_package_download_site 判断
                                        # 「原地改名」还是「新建+删旧」，
                                        # 避免把刚改好名的接口自己删掉。
                                        old_sid = str(s.get("id", ""))
                                        spider._update_package_download_site(
                                            old_sid, new_name, new_url)
                                        if old_sid in site_states:
                                            site_states.pop(old_sid, None)
                                    kit.toast("已更新")
                                    refresh_site_list()
                                except Exception as exc:
                                    kit.toast("保存失败: {}".format(exc), long=True)
